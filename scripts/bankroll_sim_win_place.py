...
def _ascii_bar_chart(balance_history: list[tuple[str, float]]) -> str:
    ...
    for ym, bal in monthly.items():
        ...
        mark = (
            "▲"
            if bal > _INITIAL_BANKROLL
            else ("▼" if bal < _INITIAL_BANKROLL else "─")
        )
...
def main() -> None:
    ...
    for model, name in [(honmei, "本命"), (manji, "卍"), (place, "複勝")]:
        try:
            ...
            print(
                f"⚠️  {name}モデルが見つかりません。先に scripts/run_train.py を実行してください"
            )

    print(f"\n=== 所持金推移シミュレーション {args.year_from}-{args.year_to} ===")
    print(
        f"初期資金: ¥{_INITIAL_BANKROLL:,.0f}  固定賭け金: ¥{_BET_AMOUNT:,.0f}  EV閾値: {args.ev_threshold}"
    )
    print("計算中 (数分かかります)...")

    res = _simulate(
        conn, honmei, manji, place, args.year_from, args.year_to, args.ev_threshold
    )
    conn.close()
    ...
    for key, d in res["detail"].items():
        ...
        print(
            f"  {label}: {d['bets']:>4}回  的中率{hr_d:5.1f}%  ROI{roi_d:6.1f}%  損益{d['payout'] - d['invested']:+,.0f}円"
        )
