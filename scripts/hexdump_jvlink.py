    ...
    for start in range(36, 80):
        ...
        try:
            ...
            if any(
                ...
                print(
                    f"  [{start}:{start + 36}] 馬名候補(SJIS): {decoded.rstrip(chr(0)).strip()!r}"
                )
                break
        except Exception:
            pass
...
def analyze_hr_offsets(raw: bytes) -> None:
    ...
    print(
        "  offset27〜 asc: "
        + "".join(chr(b) if 0x20 <= b < 0x7F else "." for b in section)
    )
    ...
    for entry_size, combo_len, amount_len, pop_len in [
        ...
        for i in range(3):
            ...
            try:
                ...
                amt_int = -1
            print(
                f"    #{i + 1}: combo={combo!r} amt={amount!r}({amt_int:,}) pop={pop!r}"
            )
            pos += entry_size
