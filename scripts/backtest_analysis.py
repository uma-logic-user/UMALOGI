...
print("=" * 70)
print(
    f"{'券種':<8}{'件数':>6}{'的中':>6}{'的中率':>7}{'投資':>10}{'払戻':>10}{'損益':>10}{'ROI':>8}"
)
print("-" * 70)
...
for bt, grp in df.groupby("bet_type"):
    ...
    print(
        f"{bt:<8}{cnt:>6,}{hit:>6,}{hit / cnt * 100:>7.1f}%{inv:>10,.0f}{pay:>10,.0f}{pro:>10,.0f}{roi:>7.1f}%"
    )
...
for thr in thresholds:
    ...
    hit = sub["is_hit"].sum()
    print(
        f"{thr:>6.1f}{len(sub):>6,}{hit:>6,}{inv:>10,.0f}{pay:>10,.0f}{roi:>7.1f}%{pro:>10,.0f}"
    )
...
print("=" * 70)
print(
    f"{'券種':<8}{'件数':>5}{'EV平均':>8}{'EV中央':>8}{'EV最大':>8}{'EV<1.0':>8}{'EV1.0-1.2':>11}{'EV≥1.2':>8}"
)
print("-" * 70)
for bt in bet_types_order:
    grp = df[df["bet_type"] == bt]
    ...
    n = len(grp)
    print(
        f"{bt:<8}{n:>5,}{grp['ev'].mean():>8.3f}{grp['ev'].median():>8.3f}{grp['ev'].max():>8.3f}"
...
for bt in bet_types_order:
    ...
    if len(grp) == 0:
        ...
        continue
    print(
        f"{bt:<8}: 的中{len(grp):>3}件  平均払戻 ¥{grp['payout'].mean():>8,.0f}  "
...
print("【モデル別 ROI】")
print("=" * 70)
for mt_label, mask in [
    ("本命モデル", df["model_type"].str.startswith("本命")),
    ...
    roi = pay / inv * 100 if inv else 0
    print(
        f"{mt_label}: {len(grp):>5,}件  ROI={roi:.1f}%  的中={grp['is_hit'].sum()}/{len(grp)}({grp['is_hit'].mean() * 100:.1f}%)"
...
for label, kw in [("暫定予想", "暫定"), ("直前予想", "直前")]:
    ...
    roi = pay / inv * 100 if inv else 0
    print(
        f"{label}: {len(grp):>5,}件  ROI={roi:.1f}%  的中={grp['is_hit'].sum()}/{len(grp)}({grp['is_hit'].mean() * 100:.1f}%)"
        f"  投資¥{inv:,.0f}  払戻¥{pay:,.0f}"
    )
...
strategies = [
    ...
    (
        "EV≥1.0 ワイドのみ",
        df[(df["ev"] >= 1.0) & (df["bet_type"] == "ワイド")],
        "ワイド",
    ),
    (
        "EV≥1.1 単勝+複勝",
        df[(df["ev"] >= 1.1) & (df["bet_type"].isin(["単勝", "複勝"]))],
        "単複",
    ),
    (
        "EV≥1.1 単勝+ワイド",
        df[(df["ev"] >= 1.1) & (df["bet_type"].isin(["単勝", "ワイド"]))],
        "単勝+ワイド",
    ),
    (
        "EV≥1.2 単勝+複勝+ワイド",
        df[(df["ev"] >= 1.2) & (df["bet_type"].isin(["単勝", "複勝", "ワイド"]))],
        "単複ワイド",
    ),
    (
        "EV≥1.0 馬連+ワイド",
        df[(df["ev"] >= 1.0) & (df["bet_type"].isin(["馬連", "ワイド"]))],
        "馬連ワイド",
    ),
    (
        "卍 EV≥1.1 単勝+複勝",
        df[
            (df["model_type"].str.startswith("卍"))
            & (df["ev"] >= 1.1)
            & (df["bet_type"].isin(["単勝", "複勝"]))
        ],
        "卍単複",
    ),
]
...
print(
    f"{'戦略':<30}{'件数':>6}{'的中':>5}{'投資':>10}{'払戻':>10}{'ROI':>8}{'損益':>10}"
)
print("-" * 80)
for name, sub, _ in strategies:
    ...
    print(
        f"{name:<30}{len(sub):>6,}{hit:>5,}{inv:>10,.0f}{pay:>10,.0f}{roi:>7.1f}%{pro:>10,.0f}{mark}"
    )
