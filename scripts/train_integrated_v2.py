...
def build_person_lookup(conn: sqlite3.Connection) -> pd.DataFrame:
    ...
    if not rows:
        ...
        return pd.DataFrame(
            columns=["race_id", "horse_number",
                     ...
                     "jockey_starts_90d",  "trainer_starts_90d"]
        )
    ...
    def _rolling_rate(person_col: str) -> pd.DataFrame:
        ...
        for _, row in target.iterrows():
            ...
            if person in person_records:
                ...
                lo = int(np.searchsorted(dates_arr, np.datetime64(window_start, "D"), side="left"))
                hi = int(np.searchsorted(dates_arr, np.datetime64(race_date,    "D"), side="left"))
                n_starts = hi - lo
    ...
    merged = jockey_df.merge(
        trainer_df[["race_id", "horse_number",
                    "trainer_win_rate_90d", "trainer_starts_90d"]],
...
def build_integrated_df(
    ...
    for yr in years:
        ...
        if rr_rows:
            rr_df = pd.DataFrame(
                rr_rows, columns=["race_id", "horse_number", "gate_number", "weight_carried"]
            )
        ...
        logger.info(
            ...
            yr, len(df), df["race_id"].nunique(), df["is_winner"].sum(),
...
def simulate_race(
    ...
    gate_map = dict(zip(pred["horse_number"].astype(int), pred["gate_number"].astype(int)))
...
def main() -> None:
    ...
    print(f"  特徴量: nb_win_odds + JVLink + 騎手/調教師90日勝率")
    print(f"  Edge フィルター: {'有効 (threshold=' + str(EDGE_THRESHOLD) + ')' if use_edge else '無効'}")
    print("=" * 70)
    ...
    for fold_no, train_years, test_year in folds:
        ...
        if train_df.empty:
            logger.error("学習データなし: スキップ"); continue
        ...
        if test_df.empty:
            logger.error("テストデータなし: スキップ"); continue
        ...
        logger.info(
            ...
            int(train_clean["jockey_win_rate_90d"].notna().sum()), len(train_clean)
        )
        logger.info("テスト: %d行 / %d レース", len(test_clean), test_clean["race_id"].nunique())
        logger.info(
            "  jockey_win_rate 非NaN: %d行 / %d行",
            int(test_clean["jockey_win_rate_90d"].notna().sum()), len(test_clean)
        )
        ...
        if not payout_rows:
            logger.error("払戻データなし: スキップ"); continue
        ...
        if use_edge:
            ...
            logger.info(
                "Edge >= %.2f: %d / %d 馬 (%.1f%%)",
                EDGE_THRESHOLD, edge_ok, edge_total,
        ...
        for i, race_id in enumerate(race_ids):
            ...
            for bt, (inv, ret) in simulate_race(pred_race, payout_race, use_edge).items():
                all_records.append({
                    "year":     test_year,
                    "race_id":  race_id,
                    "bet_type": bt,
        ...
        logger.info("フォールド %d 完了 (%s) — スキップ=%d", fold_no, test_year, skipped)
    ...
    print(f"  特徴量: nb_win_odds + JVLink + jockey/trainer 90日勝率  |  選択: {edge_label}")
    print("═" * 95)
    ...
    print(f"{'年度':<6} {'券種':<6} {'TOP1払戻':>12} {'TOP1寄与':>10} {'除外ROI':>10} {'通常ROI':>9}")
    print("-" * 65)
    for y in tested_years:
        for bt in ALL_BET_TYPES:
            s = all_stats.get((y, bt))
            if not s:
                continue
            flag_ex = "✅" if s["roi_ex"] >= 100 else ("🔶" if s["roi_ex"] >= 80 else "❌")
            print(
    ...
    profitable = [(s["year"], s["bet_type"], s["roi"]) for s in summary_rows if s["roi"] >= 100]
    if profitable:
        print("  ✅ ROI > 100% 達成:")
