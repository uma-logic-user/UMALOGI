...
def parse_with_spec(data: bytes, spec: list, label: str) -> None:
    ...
    for bet_type, n_entries, combo_b, amount_b, pop_b in spec:
        ...
        hex_preview = " ".join(
            f"{b:02X}" for b in section_raw[: min(30, len(section_raw))]
        )
        asc_preview = "".join(
            chr(b) if 0x20 <= b < 0x7F else "."
            for b in section_raw[: min(30, len(section_raw))]
        )
        if valid:
            print(
                f"  [{bet_type:4s}] ({n_entries}件 × {entry_size}bytes={n_entries * entry_size}) 有効{len(valid)}件 →",
                " | ".join(f"combo={c} ¥{a:,}(pop={p})" for c, a, p in valid),
            )
        else:
            print(
                f"  [{bet_type:4s}] ({n_entries}件 × {entry_size}bytes={n_entries * entry_size}) 有効なし  hex={hex_preview}"
            )
...
def main() -> None:
    ...
    with JVLinkClient(sid) as client:
        ...
        for _ in range(500000):
            ...
            if is_good_hr(data):
                found.append(data)
                print(
                    f"良好HR発見 #{len(found)} (scan={scan}, len={len(data)}, "
    ...
    for idx, data in enumerate(found[:3]):
        print(f"\n{'=' * 70}")
        print(
            f"HR #{idx + 1}  {len(data)} bytes  race={data[11:27].decode('ascii', 'replace')}  cat={chr(data[2])}"
        )
        ...
        # 両スペックで比較
        parse_with_spec(
            data, SPEC_OFFICIAL, "JV-Data 4.5.2 公式(枠連combo=2,三連pop=3)"
        )
