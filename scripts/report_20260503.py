...
def main() -> None:
    ...
    print("\n【モデル別】")
    print(
        f"  {'モデル':<20} {'賭':>6} {'的中':>5} {'的中%':>7} {'投資':>10} {'払戻':>10} {'ROI':>7} {'損益':>10}"
    )
    for key, d in sorted(by_model.items()):
        ...
        print(
            f"  {key:<20} {n:>6,} {h:>5,} {hr:>6.1f}% {inv:>10,.0f} {pay:>10,.0f} {roi:>6.1f}% {pay - inv:>+10,.0f}"
        )

    print("\n【会場別】")
    print(
        f"  {'会場':<6} {'賭':>6} {'的中':>5} {'的中%':>7} {'投資':>10} {'払戻':>10} {'ROI':>7} {'損益':>10}"
    )
    for venue, d in sorted(by_venue.items()):
        ...
        hr = h / n * 100 if n else 0
        print(
            f"  {venue:<6} {n:>6,} {h:>5,} {hr:>6.1f}% {inv:>10,.0f} {pay:>10,.0f} {roi:>6.1f}% {pay - inv:>+10,.0f}"
        )
