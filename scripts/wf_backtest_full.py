...
_BASE_FEAT: list[str] = [
    ...
    "distance", "gate_number", "horse_number", "race_number",
    "surface_code", "condition_code", "sex_code", "venue_encoded",
    "jockey_code_encoded", "trainer_code_encoded",
    "last_tc_4f", "last_tc_lap", "last_hc_4f", "last_hc_lap",
    "tc_4f_rank", "horse_age", "field_size",
]
...
def _load_mart_raw(conn: sqlite3.Connection, min_date: str, max_date: str) -> pd.DataFrame:
...
def _apply_encode(df: pd.DataFrame, maps: _EncMaps) -> pd.DataFrame:
    ...
    df["jockey_code_encoded"]  = df["jockey_code"].astype(str).map(maps.jockey).fillna(-1).astype("int16")
    df["trainer_code_encoded"] = df["trainer_code"].astype(str).map(maps.trainer).fillna(-1).astype("int16")

    df["win_odds_safe"] = pd.to_numeric(df["win_odds"],   errors="coerce").fillna(99.9).clip(lower=1.01).astype("float32")
    df["popularity"]    = pd.to_numeric(df["popularity"], errors="coerce").fillna(-1).astype("float32")
    ...
    df["field_size"] = df.groupby("race_id")["horse_number"].transform("count").astype("int8")

    try:
        year_col = df["date"].str[:4].astype(float)
    except Exception:
        year_col = pd.Series(2025.0, index=df.index)
    df["horse_age"] = (year_col - pd.to_numeric(df["birth_year"], errors="coerce").fillna(2020)).astype("float32")

    for col in ["horse_weight", "horse_weight_diff", "weight_carried", "distance",
                "gate_number", "horse_number", "race_number",
                "last_tc_4f", "last_tc_lap", "last_hc_4f", "last_hc_lap"]:
...
def _load_mart(conn: sqlite3.Connection, min_date: str, max_date: str,
               maps: Optional["_EncMaps"] = None) -> pd.DataFrame:
    ...
    if maps is None:
        ...
        _maps = _EncMaps(
            ...
            jockey  = {v: i for i, v in enumerate(sorted(raw["jockey_code"].dropna().unique(), key=str))},
            trainer = {v: i for i, v in enumerate(sorted(raw["trainer_code"].dropna().unique(), key=str))},
        )
    else:
        _maps = maps
...
def _load_payouts(conn: sqlite3.Connection, race_ids: list[str]) -> dict[str, dict[str, dict[str, float]]]:
    """
...
def _fit_classifier(
    ...
    clf.fit(
        ...
        callbacks=[lgb.early_stopping(_EARLY_STOP, verbose=False), lgb.log_evaluation(-1)],
    )
    return clf
...
def _simulate_bets(
    ...
    for race_id, grp in test_df.groupby("race_id"):
        ...
        if bet_type == "単勝":
            ...
            n_bets += 1; total_invest += 100
            if pay > 0:
                n_hits += 1; total_payout += pay
...
def run_walkforward(
    ...
    print(f"venue={len(enc_maps.venue)} jockey={len(enc_maps.jockey)} trainer={len(enc_maps.trainer)}")
    ...
    for i, (ts, te, tc) in enumerate(windows):
        month_label = ts[:7]
        print(f"[{i+1:02d}/{len(windows)}] テスト月: {month_label}  (学習カットオフ: {tc})", flush=True)
        ...
        if _train_cache is None:
            ...
            diff_start = (date.fromisoformat(_cache_cutoff) + timedelta(days=1)).isoformat()
            if diff_start <= tc:
                delta_raw = _load_mart_raw(conn, diff_start, tc)
                if not delta_raw.empty:
                    delta_enc = _apply_encode(delta_raw, enc_maps)
                    _train_cache = pd.concat([_train_cache, delta_enc], ignore_index=True)
                    del delta_enc
                del delta_raw
                gc.collect()
        _cache_cutoff = tc
        ...
        for mname, pred_col, auc_val in models:
            ...
            for bt in bet_types_all:
                ...
                records.append(MonthlyRecord(
                    month=month_label, model=mname, bet_type=bt,
                    n_bets=nb, n_hits=nh, invest=inv, payout=pay,
                    roi=roi, hit_rate=hr, auc=auc_val,
                ))
        ...
        # 月次サマリー（本命×単勝のみプレビュー）
        hon_tansho = [r for r in records if r.month == month_label and r.model == "本命" and r.bet_type == "単勝"]
        if hon_tansho:
            r = hon_tansho[-1]
            mark = "✅" if r.roi >= 100 else "❌"
            print(f"       本命×単勝: {r.n_bets}点 的中{r.n_hits}({r.hit_rate:.1f}%) ROI={r.roi:.1f}%{mark}  AUC={r.auc:.4f}")
...
def print_report(records: list[MonthlyRecord]) -> None:
    ...
    print(f"  モデル: 本命/複勝/卍  馬券: 全7種  固定¥100ベット")
    print(f"  ※ 月単位でモデル再学習 — カンニングなし厳守")
    print(f"{'='*W}")
    ...
    if profitable.empty:
        ...
        print(f"  {'ランク':>4} {'モデル':>6} {'券種':>5}  {'ROI':>8}  {'的中率':>7}  {'N':>7}  {'損益':>12}  {'月数':>4}")
        print(f"  {'-'*(W-4)}")
    ...
    if unprofitable.empty:
        ...
        for _, row in unprofitable.iterrows():
            roi = row["roi"]
            gap = 100 - roi
            if   roi < 60:  note = "致命的精度不足"
            elif roi < 80:  note = "JRA控除率以下"
            elif roi < 95:  note = f"あと{gap:.1f}%で損益分岐"
            else:           note = f"あと{gap:.1f}%(閾値引上げで改善可)"
            print(f"  {'✗':>4} {row['model']:>6} {row['bet_type']:>5}  ROI={roi:>7.1f}% — {note}")

    # ── Section 2: 全組み合わせ一覧 ────────────────────────────────
    print(f"\n{'='*W}")
    print("  【全組み合わせ一覧 (ROI降順)】")
    print(f"{'='*W}")
    print(f"  {'モデル':>6} {'券種':>5}  {'ROI':>8}  {'的中率':>7}  {'N':>7}  {'投資':>10}  {'払戻':>10}  {'損益':>12}")
    print(f"  {'-'*70}")
    ...
    if top3:
        ...
        for mname, bt in top3:
            subset = df[(df["model"] == mname) & (df["bet_type"] == bt)].sort_values("month")
            if subset.empty:
                continue
            ...
            print(f"  {'月':>8}  {'点数':>6}  {'的中':>5}  {'ROI':>8}  {'月損益':>12}  {'累計':>12}")
            cum = 0.0
    ...
    if best is not None and best["roi"] >= 100:
        ...
        if best["bet_type"] in ("単勝", "複勝"):
            print(f"  → {best['model']}モデルで EV ≥ 1.3 以上の馬のみ{best['bet_type']}を購入。")
            print(f"    JRA 控除率を超えるアルファを持つ組み合わせとして最推奨。")
        elif best["bet_type"] in ("三連複", "三連単", "馬連"):
            print(f"  → {best['model']}モデルの予測順位上位馬で{best['bet_type']}を形成。")
            print(f"    全レース機械的購入（1票/レース）で高ROIを確認。")
    else:
        print("\n  全組み合わせ ROI < 100%。")
...
def main() -> None:
    ...
    parser.add_argument("--quick",     action="store_true", help="各月1000行に制限（デバッグ）")
    parser.add_argument("--out",       default="data/wf_backtest_full.json", help="出力JSONパス")
    args = parser.parse_args()

    print(f"\n{'='*65}")
    print("  UMALOGI 全券種 Walk-Forward バックテスト")
    print(f"  期間: {args.start} 〜 {args.end}")
    print(f"  カンニング完全排除 / 固定¥100ベット / 全7券種")
    if args.quick:
