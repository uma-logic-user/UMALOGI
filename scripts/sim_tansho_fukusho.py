...
def main() -> None:
    ...
    print(
        f"  payouts: {len(payouts_raw):,}行 / {payouts_raw['race_id'].nunique():,}レース",
        flush=True,
    )
                ...
                rows.append(
                    {
                        ...
                        "invested": UNIT,
                        "payout_yen": pr_ev1 * UNIT / 100
                        if hit_ev1 and pr_ev1 > 0
                        else 0,
                    }
                )
                # 複勝
                ...
                rows.append(
                    {
                        ...
                        "invested": UNIT,
                        "payout_yen": pr_ev3 * UNIT / 100
                        if hit_ev3 and pr_ev3 > 0
                        else 0,
                    }
                )
    ...
    for strategy in ["全賭け", "EV>1"]:
        ...
        print(
            f"{'モデル':<4} {'券種':<4} {'賭数':>6} {'的中':>5} {'的中率':>7} {'投資額':>10} {'払戻':>10} {'ROI':>7} {'損益':>10}"
        )
        for (mdl, bt), g in sub.groupby(["model", "bet_type"]):
            n = len(g)
            h = g["is_hit"].sum()
            inv = g["invested"].sum()
            pay = g["payout_yen"].sum()
            roi = pay / inv * 100 if inv > 0 else 0.0
            print(
                f"{mdl:<4} {bt:<4} {n:>6,} {h:>5,} {h / n * 100:>6.1f}% {inv:>10,.0f} {pay:>10,.0f} {roi:>6.1f}% {pay - inv:>+10,.0f}"
            )
    ...
    for strategy in ["全賭け", "EV>1"]:
        ...
        for mdl in ["本命", "卍"]:
            for bt in ["単勝", "複勝"]:
                ...
                print(
                    f"  {mdl}/{bt}: 賭数={len(sub):,} 最終残高={final:,.0f}円 "
