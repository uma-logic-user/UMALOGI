...
def _already_scraped_in_range(from_date: str, to_date: str) -> int:
    ...
    res_conn.execute(f"ATTACH DATABASE '{str(_MAIN_DB).replace(chr(92), '/')}' AS mdb")
...
def run_scrape() -> bool:
    ...
    print(
        f"\n[完了] スクレイプ終了  ok={ok}件 / err={err}件  経過={elapsed_total:.1f}分"
    )
    return True
...
def main() -> None:
    ...
    parser.add_argument(
        "--skip-scrape", action="store_true", help="スクレイプをスキップ"
    )
    parser.add_argument(
        "--skip-train", action="store_true", help="ALPHA再学習・バックテストをスキップ"
    )
