...
def analyze(raw: bytes) -> None:
    ...
    horse_name = (
        horse_name_raw.decode("cp932", errors="replace").replace("\x00", "").strip()
    )
    ...
    for off in range(80, 125):
        ...
        if off + 3 <= len(raw):
            ...
            if all(0x30 <= x <= 0x39 for x in tri):
                ...
                if 480 <= val <= 620:
                    print(
                        f"    bytes[{off}:{off + 3}] = {tri.decode()} → 斤量候補 {val / 10:.1f}kg (×10)"
                    )
        ...
        if off + 2 <= len(raw):
            ...
            if all(0x30 <= x <= 0x39 for x in pair):
                ...
                if 0 <= val <= 15:
                    print(
                        f"    bytes[{off}:{off + 2}] = {pair.decode()} → 斤量候補 {val + 50}.0kg(ASCII+50)"
                    )
