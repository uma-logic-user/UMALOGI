...
def try_parse(data: bytes, specs: list, label: str) -> None:
    ...
    for bet_type, n_ent, cb, pb in specs:
        ...
        print(
            f"  {bet_type:5s} n={n_ent} ent={ent_len}b off={offset}: {valid if valid else '(none)'}"
        )
            ...
            for _ in range(1_000_000):
                ...
                try:
                    ...
                    if tan > 0:
                        ...
                        cat = chr(data[2])
                        print(
                            f"HR発見 #{len(found)} scan={scan} race={race_id} cat={cat} len={len(data)} TAN_AMT={tan}"
                        )
                        if len(found) >= 3:
                            break
                except Exception:
