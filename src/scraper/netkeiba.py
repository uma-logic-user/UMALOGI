...
def _parse_results_table(
    ...
    for tr in table.select("tr.HorseList"):
        ...
        horse_name = (
            horse_link.get_text(strip=True) if horse_link else cells[_COL_HORSE_NAME]
        )
        horse_id = ""
...
def fetch_race_results(
    ...
    *,
    ...
    for horse_name, horse_id, cells in raw_rows:
        ped = (
            _fetch_pedigree(horse_id, delay=delay, session=session)
        ...
        results.append(
            HorseResult(
                ...
                gate_number=_parse_int(cells[_COL_GATE_NUMBER])
                if len(cells) > _COL_GATE_NUMBER
                else None,
                horse_number=_parse_int(cells[_COL_HORSE_NUMBER])
                if len(cells) > _COL_HORSE_NUMBER
                else None,
                sex_age=cells[_COL_SEX_AGE] if len(cells) > _COL_SEX_AGE else "",
                weight_carried=_parse_float(cells[_COL_WEIGHT]) or 0.0
                if len(cells) > _COL_WEIGHT
                else 0.0,
                jockey=cells[_COL_JOCKEY] if len(cells) > _COL_JOCKEY else "",
                trainer=cells[_COL_TRAINER] if len(cells) > _COL_TRAINER else "",
                finish_time=cells[_COL_TIME] or None
                if len(cells) > _COL_TIME
                else None,
                margin=cells[_COL_MARGIN] or None if len(cells) > _COL_MARGIN else None,
                win_odds=_parse_float(cells[_COL_WIN_ODDS])
                if len(cells) > _COL_WIN_ODDS
                else None,
                popularity=_parse_int(cells[_COL_POPULARITY])
                if len(cells) > _COL_POPULARITY
                else None,
...
def _combo_li_nums(td, bet_type: str) -> list[list[int]]:
    ...
    if uls:
        for ul in uls:
            ...
            for li in ul.find_all("li", recursive=False):
                li_nums = [
                    int(n)
                    for n in re.findall(r"\d+", li.get_text())
                    if 0 < int(n) <= max_num
                ]
                nums.extend(li_nums)
    ...
    all_nums = [
        int(n) for n in re.findall(r"\d+", td.get_text()) if 0 < int(n) <= max_num
    ]
    step = combo_size[0]
    if step > 0:
...
def _validate_payout_record(
    bet_type: str, combo: str, payout: int, pop: Optional[int]
) -> bool:
    """
    ...
    if payout < 100:
        logger.warning(
            "払戻不正: bet_type=%s combo=%s payout=%d (< 100)", bet_type, combo, payout
        )
        return False
    # 最大払戻（三連単 1000 万超はありえない）
    if payout > 10_000_000:
        logger.warning(
            "払戻不正: bet_type=%s combo=%s payout=%d (> 10M)", bet_type, combo, payout
        )
        return False
    ...
    if not (sizes[0] <= len(parts) <= sizes[1]):
        logger.warning(
            "組み合わせ馬番数不正: bet_type=%s combo=%s (expected %s nums)",
            bet_type,
            combo,
            sizes,
        )
    ...
    for p in parts:
        if not p.isdigit() or not (1 <= int(p) <= max_num):
            logger.warning(
                "馬番不正: bet_type=%s combo=%s (part=%r, max=%d)",
                bet_type,
                combo,
                p,
                max_num,
        ...
        for tr in table.find_all("tr"):
            ...
            if len(tds) > 2:
                ...
                if pop_spans:
                    ...
                    pops = [
                        _parse_payout_int(re.sub(r"[^\d,]", "", t)) for t in pop_rows
                    ]
            else:
                pops = []
            ...
            if not combo_rows and bet_type in ("単勝", "複勝"):
                ...
                for i, row_text in enumerate(raw_rows):
                    nums = [
                        int(x) for x in re.findall(r"\d+", row_text) if 0 < int(x) <= 18
                    ]
                    if not nums:
                        continue
    ...
    for table in soup.select("table.pay_table_01, table.pay_table_02"):
        for tr in table.select("tr"):
            ...
            for i, raw_combo in enumerate(combo_rows):
                combo = _normalize_combination(
                    BeautifulSoup(raw_combo, "lxml").get_text()
                )
                if not combo:
                    continue
                # 番号バリデーション
                ...
                if not (combo_size[0] <= len(parts) <= combo_size[1]):
                    logger.warning(
                        "旧形式 組み合わせ不正: bet_type=%s raw=%r", bet_type, raw_combo
                    )
                    continue
...
def fetch_race_payouts(
    ...
    *,
    ...
    if soup.select("table.Payout_Detail_Table"):
        ...
        logger.debug(
            "払戻取得 (Payout_Detail_Table): race_id=%s, %d 件",
            race_id,
            len(new_results),
        )
    ...
    if old_results:
        ...
        for r in old_results:
            ...
            if key not in existing:
                ...
                existing.add(key)
        logger.debug(
            "払戻取得 (pay_table): race_id=%s, +%d 件追加", race_id, len(old_results)
        )
