...
def train_honmei_model(train_df: pd.DataFrame) -> tuple:
    ...
    logger.info("LightGBM 訓練: %d レース / %d サンプル / %d-fold CV",
                n_races, len(X), n_splits)
...
def simulate_race(
    ...
    if n >= 3 and edge_top >= TRIO_EDGE_THRESHOLD:
        ...
        bet_trio    = kelly_bet(bankroll, p_trio, "三連複", n, remaining,
                                edge_threshold=TRIO_EDGE_THRESHOLD)
        if bet_trio > 0:
...
def print_report(df: pd.DataFrame, cv_auc: float) -> None:
    ...
    print(f"  Edge 閾値             : {EDGE_THRESHOLD}× (1/{int(EDGE_THRESHOLD)}以上の有利度のみ)")
    ...
    for bt in ["単勝", "複勝", "馬連", "ワイド", "三連複"]:
        ...
        pnl  = out - inv
        mxp  = float(sub.loc[sub["is_hit"] == 1, "payout_per100"].max()) if hit > 0 else 0.0
    ...
    # ── 月次推移 ──
    print(f"\n■ 月次 ROI 推移")
    print(f"{'月':>4}{'件数':>5}{'的中':>5}{'投資額':>10}{'ROI':>7}{'月次損益':>10}{'月末残高':>12}")
    print("-" * 58)
    ...
    for month in sorted(df["month"].unique()):
        ...
        if not bl_df.empty:
            ...
            last_bl = 0.0
        print(f"{month:>4}月{len(msub):>5,}{mhit:>5,}{minv:>10,.0f}{mroi:>6.1f}%{mpnl:>10,.0f}{last_bl:>12,.0f}")
    ...
    for thr in [1.1, 1.3, 1.5, 2.0, 3.0]:
        ...
        print(f"{thr:>6.1f}x{len(sub):>5,}{hit:>5,}{inv:>10,.0f}{roi:>6.1f}%{pnl:>10,.0f}{mark}")
    ...
    print(f"  2. 単勝オッズ推定値: {TYPICAL_ODDS['単勝']}倍、馬連: {TYPICAL_ODDS['馬連']}倍、三連複: {TYPICAL_ODDS['三連複']}倍")
...
def main() -> None:
    ...
    logger.info("Train サンプル数: %d  (レース=%d)", len(train_df), train_df["race_id"].nunique())
