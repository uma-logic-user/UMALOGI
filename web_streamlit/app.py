...
# ── Analytics 派生データ（キャッシュ済み計算）───────────────────────

def _kelly_simulate_core(
    bets_arr: "np.ndarray",
    payouts_arr: "np.ndarray",
    hits_arr: "np.ndarray",
    dates_arr: "np.ndarray",
    initial_bankroll: float = 1_000_000.0,
) -> dict[str, Any]:
    """Kelly シミュレーションの純粋計算コア（Streamlit 非依存・テスト可能）。

    iterrows を廃止し numpy 配列ベースでアクセスする。
    bankroll の 10% キャップは逐次依存のため Python ループで処理するが、
    wins 集計は hits_arr.sum() でベクトル化済み。
    """
    n = len(bets_arr)
    br_arr: np.ndarray = np.empty(n + 1, dtype=np.float64)
    dt_arr: list[Any]  = [None] * (n + 1)
    br_arr[0] = initial_bankroll
    dt_arr[0] = dates_arr[0] if n > 0 else None

    bankroll = np.float64(initial_bankroll)
    peak     = bankroll
    max_dd   = np.float64(0.0)

    for i in range(n):
        cap        = bankroll * np.float64(0.10)
        actual_bet = bets_arr[i] if bets_arr[i] < cap else cap
        if actual_bet <= 0.0 or bankroll <= 0.0:
            br_arr[i + 1] = bankroll
            dt_arr[i + 1] = dates_arr[i]
            continue
        profit   = (payouts_arr[i] - actual_bet) if hits_arr[i] else -actual_bet
        bankroll += profit
        if bankroll > peak:
            peak = bankroll
    ...
    series_df = pd.DataFrame({"date": pd.to_datetime(dt_arr), "bankroll": br_arr})
    return {
        "empty":     False,
        "series_df": series_df,
        "bankroll":  float(bankroll),
        "max_dd":    float(max_dd),
        "wins":      int(hits_arr.sum()),   # ← ベクトル化集計
        "total":     n,
    }


@st.cache_data(ttl=300)
def _build_monthly_total(kind: str = "all") -> tuple[pd.DataFrame, pd.DataFrame]:
    """月次 ROI DataFrame と集計済み monthly_total を返す（派生計算をキャッシュ）。"""
    monthly_df = fetch_monthly_roi(kind=kind)
    if monthly_df.empty:
    ...
    return monthly_df, monthly_total


@st.cache_data(ttl=300)
def _build_kelly_series(kind: str = "all") -> dict[str, Any]:
    """Kelly シミュレーション系列を計算してキャッシュする。"""
    sim_df = fetch_kelly_simulation(kind=kind)
    if sim_df.empty:
        return {"empty": True}
    return _kelly_simulate_core(
        bets_arr    = sim_df["recommended_bet"].to_numpy(dtype=np.float64),
        payouts_arr = sim_df["payout"].to_numpy(dtype=np.float64),
        hits_arr    = sim_df["is_hit"].to_numpy(dtype=bool),
        dates_arr   = sim_df["created_at"].to_numpy(),
    )


@st.cache_data(ttl=300)
def _build_venue_stats(kind: str = "all") -> pd.DataFrame:
    """会場別成績 DataFrame（ROI・的中率計算済み）をキャッシュして返す。"""
    venue_df = fetch_venue_performance(kind=kind)
    if venue_df.empty:
        return venue_df
    ...
    return venue_df.sort_values("roi", ascending=False)
...
def render_prerace_table(df: pd.DataFrame) -> None:
    ...
    # ── ベクトル化列生成（apply(axis=1) を廃止） ───────────────────
    # EV 数値列（expected_value → ev_score フォールバック）
    _ev_num = (
        display["expected_value"]
        .fillna(display["ev_score"].fillna(0))
    )
    _bet = display["recommended_bet"]

    # kelly_bet: EV>=1.0 かつ bet>0 の行だけ金額文字列、それ以外 "—"
    _kelly_mask = (_bet > 0) & pd.notna(_bet) & (_ev_num >= 1.0)
    _kelly_vals = pd.Series("—", index=display.index, dtype=object)
    if _kelly_mask.any():
        _kelly_vals[_kelly_mask] = _bet[_kelly_mask].astype(int).map(lambda x: f"¥{x:,}")
    display["kelly_bet"] = _kelly_vals

    display["調教"] = display.get("eval_grade", pd.Series(dtype=str)).apply(_eval_badge)

    # EV 表示列: NA は "—"、>=1.0 は 🔥 prefix
    _ev_raw_na = pd.isna(display["expected_value"]) & pd.isna(display["ev_score"])
    display["EV"] = np.where(
        _ev_raw_na,
        "—",
        np.where(
    ...
    display["_ev_num"] = _ev_num

    # 本命% / 単勝: Series.map（単列演算・apply より高速）
    display["本命%"] = display["model_score"].map(
        lambda x: f"{x*100:.1f}%" if pd.notna(x) else "—"
    )
    display["単勝"] = display["win_odds"].map(
        lambda x: f"{x:.1f}" if pd.notna(x) else "—"
    )

    # 体重: np.where で NaN 判定 → 文字列結合（apply(axis=1) を廃止）
    _w = pd.to_numeric(
        display.get("horse_weight",      pd.Series(np.nan, index=display.index)),
        errors="coerce",
    )
    _d = pd.to_numeric(
        display.get("horse_weight_diff", pd.Series(np.nan, index=display.index)),
        errors="coerce",
    )
    _w_has  = pd.notna(_w)
    _d_suf  = pd.Series("", index=display.index, dtype=object)
    _d_has  = pd.notna(_d)
    if _d_has.any():
        _d_suf[_d_has] = _d[_d_has].astype(int).map(lambda x: f"({x:+d})")
    display["体重"] = np.where(
        _w_has,
        _w.where(_w_has).fillna(0).astype(int).astype(str) + _d_suf,
        "—",
    )

    display["着順"] = display.get("rank", pd.Series(dtype=str)).map(
        lambda x: f"{int(x)}着" if pd.notna(x) else "—"
    )
    ...
    if not hot_rows.empty:
        ...
        for h in hot_rows.sort_values("_ev_num", ascending=False).to_dict("records"):
            ev    = h["_ev_num"]
            odds  = h.get("win_odds")
...
def render_bets_table(bets_df: pd.DataFrame, tab_key: str = "") -> None:
    ...
    if not hot.empty:
        ...
        for row in hot.to_dict("records"):
...
def render_analytics() -> None:
    ...
    monthly_df, monthly_total = _build_monthly_total(kind=kind_key)
    if monthly_df.empty:
        st.warning("月次データなし")
        return
    ...
    with tab_kelly:
        kelly = _build_kelly_series(kind=kind_key)
        if kelly.get("empty"):
            st.info("EV >= 1.0 のベット履歴がありません")
        else:
            series_df = kelly["series_df"]
    ...
    with tab_venue:
        venue_df = _build_venue_stats(kind=kind_key)
        if venue_df.empty:
            st.info("会場別データなし")
        else:
            fig_v = px.bar(
...
def render_hit_performance() -> None:
    ...
    disp["結果"] = np.where(disp["is_hit"] == 1, "✅ 的中", "❌ 外れ")
    # payout_str: 的中かつ payout あり → 金額文字列、それ以外 "—"
    _payout_mask = (disp["is_hit"] == 1) & pd.notna(disp["payout"])
    disp["payout_str"] = "—"
    if _payout_mask.any():
        disp.loc[_payout_mask, "payout_str"] = (
            disp.loc[_payout_mask, "payout"].astype(int).map(lambda x: f"¥{x:,}")
        )
        ...
        if not dates:
            ...
            if not venues:
                ...
                if races_df.empty:
                    ...
                    # _race_label を apply(axis=1) から列演算に変換
                    _rn     = races_df["race_number"].astype(int).map(lambda x: f"{x:02d}R")
                    _rname  = races_df["race_name"].map(_safe_race_name)
                    _labels = np.where(
                        _rname.notna() & (_rname != ""),
                        _rn + "  " + _rname.fillna(""),
                        _rn,
                    )
                    race_options = dict(zip(_labels, races_df["race_id"]))
                    with nav_c3:
