...
import { rowToObj, sortedCombinations, chunkArray, identifyBetForm } from '@/lib/dbHelpers'
...
export async function GET() {
  try {
    ...
    const preds = db.prepare(`
      ...
      WHERE pr.is_hit = 1
      ORDER BY r.date DESC, p.race_id, p.id
    ...
    return NextResponse.json(validateResponse(output, '[/api/hits]'))
  } catch (err) {
    console.error('[/api/hits]', err)
