...
| 日付 | 変更内容 |
|------|---------|
| 2026-06-14 | 【Web UI（ポート3000）SNS用／個人ガチ用トラック判別表示（W-092・v1.15.1-dev）】直前予想を「📱 SNS用（Hit-Focused 多券種）」と「💰 個人ガチ用（EV-Focused 実弾単複）」に一瞬で見分けられるよう、タブ切替＋各行バッジを実装。**分類はサーバー側を単一真実源**とし、`web/src/lib/dbHelpers.ts` に純関数 `classifyTrack(modelType, betType): 'sns'|'gachi'`（**ハイブリッド・ルール**: `_v0525` を含む→'sns' ／ 券種が単勝・複勝→'gachi' ／ それ以外の馬連・馬単・ワイド・三連複・三連単→'sns'）を追加。`/api/predictions` の各行へ `track` を付与し、`web/src/types/race.ts:Prediction` に `track?: 'sns'|'gachi'` を追加。`PredictionsPanel.tsx` に①トラックタブ（全部／💰 ガチ／📱 SNS・各タブに件数バッジ）で `items` をフィルタ、②`TrackBadge`（📱 SNS=シアン／💰 ガチ=グリーン）をデスクトップ表のモデル列とモバイルカードヘッダに表示。API に `track` が無い場合もクライアント側 `trackOf()` が `classifyTrack` で再計算するフォールバック付き。**実弾で実際に賭ける単複ロックの中身は不変＝本変更は表示分類のみ**。検証: jest 32 PASS（classifyTrack 4 新規）・`tsc --noEmit` 0。影響: web/src/lib/dbHelpers.ts, web/src/app/api/predictions/route.ts, web/src/types/race.ts, web/src/components/PredictionsPanel.tsx, web/__tests__/classifyTrack.test.ts。関連: [[W-091]] [[W-092]] |
| 2026-06-12 | 【Web UI（ポート3000）プレミアムレポート統合（v1.14.0-dev）】メイン監視環境である Next.js ダッシュボード（`http://<host>:3000/`）に漆黒×シャンパンゴールドのプレミアムレポートと高EVシグナルを完全配線。①`premium_pack.py` が `premium_signals.json`（race_id/venue/race_number/race_name/max_ev/candidates[bet_type/combo/prob/odds/ev/stake]）を4ファイル目として `outputs/marketing/YYYYMMDD/` に出力。②新 API `GET /api/premium-report?date=YYYYMMDD`（省略時=最新日）が premium_sanren.html を `text/html; charset=utf-8`・`Cache-Control: no-store`・`X-Report-Date` 付きで配信（date は8桁数字のみ受理＝トラバーサル防止）。③新 API `GET /api/premium-signals` が JSON を配信（CLAUDE.md 16条の TypeScript 文字化けゲート `cleanStrings`/`isGarbled` を全文字列に適用・化け検知時は空文字置換）。④サイドバーに「💎 プレミアムレポート」ビュー新設（`PremiumReportPanel.tsx`）— RACES/SIGNALS/MAX EV サマリカード（EV≥1.42 で 🔥金色強調）＋レースごとの買い目テーブル（券種/買い目/確率/推定オッズ/EV/参考ベット額）＋HTML レポートの iframe 埋め込み（「全画面で開く↗」リンク付き）。60秒間隔で自動再取得し、週末バッチの自動生成にリアルタイム追従。影響ファイル: web/src/app/api/premium-report/route.ts(新規), web/src/app/api/premium-signals/route.ts(新規), web/src/components/PremiumReportPanel.tsx(新規), web/src/components/AppShell.tsx, src/marketing/premium_pack.py |
| 2026-06-11 | 【UI/UX 超絶強化 — CUI Rich化/Discord プレミアムEmbed/HTML ラグジュアリーレポート（v1.13.0-dev）】①**CUI**: `src/ui/console.py` 新設（`UmaConsole`=起動バナーPanel/シアン→マゼンタ→ゴールドのグラデーションプログレスバー/「🔥 EV超え買い目発見！」金色シグナルPanel/高EV候補テーブル。rich未導入・cp932端末では plain 縮退で本番を殺さない）。`src/ops/logger.py setup_logging(use_rich=True)` で RichHandler コンソール装飾（ファイルログ書式は不変・既定False）。today_auto_runner と premium_pack スキャンに配線。②**Discord**: `src/notification/embed_builder.py` 新設。`notify_prerace_result` が (a)格付け推定 `infer_grade`（G1=青0x2E5FCC/G2=赤0xD92B47/G3=緑0x1FA86B/L=紫/OP=橙・JRA配色準拠）・自信度グラデーション（スチール→ゴールド）・万馬券級EV(≥3.0)=赤橙 の優先順で Embed 色を動的決定 (b)最高EV買い目を「🎯軸馬｜🤝相手馬｜💎EV/想定オッズ」の inline 3カラムグリッドで先頭表示 (c)EV≥1.0推奨投資合計を `████░░░░░░ 40%` インジケーター（分母=UMALOGI_BANKROLL 環境変数、既定10万円）で表示。タイトルに〔G1〕バッジ。race_name は prediction パイプライン→router→notifier へ自動連携。③**配信レポート**: `premium_pack.py` に `generate_premium_html` 追加 — premium_sanren.md と同一データ・同一の誠実性注意を Tailwind CDN＋Shippori Mincho/Cormorant Garamond の漆黒×シャンパンゴールド「プライベートバンク調査レポート」デザイン HTML（premium_sanren.html）として並列出力（自己完結・noindex）。md 側もメダル🥇/サマリ行/🔥マーカーで装飾強化。モックアップ集: docs/ui_ux_upgrade_report.md。影響: src/ui/(新規), src/notification/embed_builder.py(新規), src/notification/discord_notifier.py, src/notification/router.py, src/pipeline/prediction.py, src/marketing/premium_pack.py, src/ops/logger.py, scripts/today_auto_runner.py, requirements.txt(rich追加) |
| 2026-06-11 | 【サブスク向けプレミアム買い目＋SNSチラ見せの出力フォーマット新設（v1.11.0-dev）】outputs/marketing/YYYYMMDD/ に ①premium_sanren.md=購読者限定の三連単/三連複 EV1.30超フォーメーション表（買い目/モデル確率/推定オッズ/EV/参考ベット額・誠実性注意ヘッダ必須） ②sns_teaser.md=Leak Story（リーク自己監査→棄却→OOS110%の物語）＋前回実績＋「最大EV x.xx を検知」のチラ見せ（買い目組番は絶対に非公開）。生成: src/marketing/premium_pack.py |
| 2026-06-01 | 【Streamlitダッシュボードを web_streamlit/app.py に一本化（逆統合）】重複UI資産のクリーン統合を実施。当初 `src/web/dashboard.py`（直近結果/EV上位馬/モデル別ROI）を新設したが、精査の結果**既存 `web_streamlit/app.py`（1,480行・本番起動経路 start_streamlit.bat/UMALOGI起動.bat/Dockerfile.streamlit から起動・馬場バイアス/オッズ時系列/ケリーsim/会場別/的中実績/予想アーカイブ等）の方が遥かに高機能な正本**と判明。機能喪失を避けるため**逆方向に統合**：`web_streamlit/app.py` を正本として維持し、新ダッシュボードの独自機能のみを吸収。①「🏠 サマリー」タブを新設（トップタブ先頭）し `render_home()` を配置。②`fetch_recent_results()`=直近確定レース＋1着馬、`fetch_top_ev_horses()`=最新予想日のEV上位馬（ROW_NUMBER代表馬1頭・is_superseded除外）、`fetch_model_roi()`=**正準ロジック `src/ml/pnl_accounting.compute_live_roi` を再利用した真コスト基準のモデル別ROI**（旧Analyticsの recommended_bet 基準バグを回避）を追加。既存 `_q`/`_df`+`@st.cache_data`+テストデータ除外フィルタ（正規会場01〜10）の流儀に準拠。③重複した `src/web/dashboard.py`/`dashboard_data.py`/`__init__.py` と `tests/test_dashboard*.py` を `git rm` で削除。テスト: `tests/test_streamlit_perf.py` 34→51件（移植データ関数9＋構造8追加）全PASS。本番DBでヘッドレス起動 HTTP 200/health 200 確認。影響: web_streamlit/app.py, tests/test_streamlit_perf.py, （削除）src/web/*・tests/test_dashboard*.py |
| 2026-06-01 | 【SNS集客エンジン新設（観賞用モデル直結マネタイズ）】`src/ops/sns_publisher.py` を新規構築。①`format_x_post()`=Oracle/HitFocus 買い目の X(140字最適化/プレミアム長文2000字)コピペ整形（🏇🎯絵文字＋`#競馬予想 #UMALOGI`＋本命/激熱馬展開）。②`generate_hit_flash()`/`send_hit_flash()`/`detect_and_flash()`=レース確定→評価直後に集客モデルの高ROI(≥150%)・万馬券(配当≥10000円/100円)を検知し「【UMALOGI的中速報】〇〇円的中！回収率〇〇%！」ドヤ報告を Discord 集客ch(`DISCORD_WEBHOOK_SNS`)へ Webhook 自動送信。`scripts/fetch_race_result.py` の評価後フック2箇所に `_try_sns_flash()` を配線（best-effort）。③`export_weekly_report()`/`run_weekly_report()`=週次の観賞用モデル成績(通算的中率/回収率/最高配当)を note 貼付用 Markdown `outputs/sns/weekly_report_YYYYMMDD.md` に出力。実弾モデル(本命/卍/Alpha)は `is_ornamental_model()` で集客統計から除外。テスト `tests/test_sns_publisher.py`(13)＋`tests/test_sns_detect_flash.py`(4)。影響: src/ops/sns_publisher.py(新規), scripts/fetch_race_result.py |
| 2026-05-31 | 【厳選レースSNS集客導線】Discord(note_draft)チャンネルへ「X投稿用コピペ」通知を新設。`DiscordNotifier.notify_gachi_x_post()` 追加（note_draft_url 未設定時は予想chへfallback、コードブロックで囲みそのまま貼付可）。X文面構成=`【UMA-Logic 厳選予想】[M月D日] [会場 NR「レース名」]` + 根拠短文（期待値の歪み/合意数）+ note誘導URL + 動的ハッシュタグ。ハッシュタグ＝`#YYYY年M月D日`（先頭ゼロなし）/`#会場NR`/`#レース名`（プレースホルダ除外）/`#競馬予想`/`#UMALogic`/`#期待値競馬`/`#競馬AI`/`#JRA`。note貼付用 Markdown を `dist/notes/[yyyymmdd]_[競馬場]_[R]R_note.md` に出力（タイトル+ハッシュタグ・導入文・具体的買い目・出走馬表・末尾X導線+共通タグ）。環境変数 NOTE_MYPAGE_URL / X_ACCOUNT_URL を追加。影響: src/notification/discord_notifier.py, src/ops/note_generator.py, .env.example |
| 2026-05-30 | 【文字色トークン全体を濃色化（可読性根治）】「字が薄すぎて読めない」対応として薄いグレー系トークンを全体的に濃く変更。`--text-muted` #6B6960 → #4D4B45、`--text-subtle` #9E9B93 → #6B6960（@theme と :root 両方）。:root に欠落していた `--text-subtle` を追加。これによりツリーのレース名/会場/日付/番号・各パネルの補助テキストが濃くなる。`npm run build` PASS。影響: web/src/app/globals.css |
| 2026-05-30 | 【RaceTree発走時刻まわり可読性改善】旧ダークテーマ由来の低コントラスト表示を修正。発走時刻を 9px/半透明水色(`rgba(180,200,255,0.7)`) → 12px/`--text-primary`（発走前）・`--text-muted`（終了済）に変更。カウントダウンを 9px/ネオン緑・金 → 12px太字/30分以内=`#C44040`(赤・緊急)・それ以遠=`#5B8A5B`(緑)に変更。レース番号 10px→11px。選択中レース(`.tree-race-btn.active`)のアクセントを淡い `--neon-cyan`(#C8A882) → 濃い `--neon-blue`(#8B7355)＋太字に変更。tsc --noEmit PASS。影響: web/src/components/RaceTree.tsx, web/src/app/globals.css |
| 2026-05-29 | AI予想タブのノイズフィルター追加: RaceDetail.tsx・TabView.tsx で horse_number=null/0 および win_odds>=500（センチネルオッズ）の予想を描画前に除外。影響ファイル: web/src/components/RaceDetail.tsx, web/src/components/TabView.tsx |
| 2026-05-27 | 【Claude風デザイン移植 完了（Task 1-9）】web/ フロントエンド全体をダーク・ネオン配色からウォームベージュ×セリフ体×ミニマルデザインへ移植完了。globals.css に Tailwind v4 @theme トークン13色定義・後方互換変数リマップ。NavBar/AppShell/RaceDetail/RaceTable/PredictionsPanel/BiasPanel/ConditionAnalysis/FinancialDashboard/GachiHits/HitHistory/PedigreeChart/RaceHeader/TabView/TodayBuyPanel/Win5Panel の全コンポーネントからネオン rgba / textShadow glow / boxShadow glow を一掃。tsc --noEmit PASS・npm run build PASS（Compiled successfully in 48s）。影響: web/src/app/globals.css, web/src/components/ 全15コンポーネント |
| 2026-05-27 | 【Claude風デザイン移植 Task2】globals.cssをダーク・ネオン系からウォームベージュ系へ全面置換。Tailwind v4 @themeトークン13色追加、:root後方互換変数リマップ、全neonグロー/グリッド背景を削除。影響: web/src/app/globals.css |
| 2026-05-25 | 【Streamlit ダッシュボード描画高速化】`web_streamlit/app.py` をリファクタリング。①Kelly シミュレーション `iterrows` ループを `_kelly_simulate_core()`（numpy 配列ベース）に置換、`wins` は `hits_arr.sum()` でベクトル化。② `monthly_total`・会場別 ROI の派生 DataFrame 計算を `_build_monthly_total` / `_build_venue_stats` / `_build_kelly_series` として `@st.cache_data(ttl=300)` キャッシュ化。③ `render_analytics` / `render_hit_performance` に `@st.fragment` を付与して内部 selectbox 変更時のメインスクリプト再実行を抑制。④ `render_bias_panel(selected_race_id)` をサブタブ内2回呼び出しからサブタブ外1回呼び出しに変更。⑤ `render_prerace_table` / `render_bets_table` / `race_options` 構築の全 `iterrows()` を `to_dict("records")` / `apply()` に置換。34テスト ALL PASS。影響: `web_streamlit/app.py`, `tests/test_streamlit_perf.py`（新設） |
| 2026-05-27 | 【SHAP根拠可視化モーダル（提案B）】① `src/ml/shap_explainer.py` 新設（`compute_shap_top` / `build_shap_map` / `FEATURE_LABEL_JP` 70特徴量日本語マッピング）。② `prediction_horses.shap_json` 列追加（マイグレーション #18）。③ 推論時に `shap.TreeExplainer` で全馬SHAP計算 → `build_shap_map` で `horse_number → shap_json` 変換 → `insert_prediction` 経由でDB保存。④ `GET /api/shap/[prediction_id]` エンドポイント新設（shap_json をパースし `[{feature, label, value}]` 配列に変換して返却）。⑤ `src/lib/shapLabels.ts` 新設（TS側ラベルマップ）。⑥ `TodayBuyPanel.tsx` に「🔍」ボタン（列追加）とインラインSHAPパネルを追加（Tailwind CSS水平バー・青=プラス寄与/赤=マイナス寄与・最大絶対値正規化）。⑦ `shap` 未インストール時・計算失敗時は空辞書にフォールバックし予測パイプラインを停止しない。Python pytest 13テスト ALL PASS。TypeScript tsc --noEmit PASS。影響: `src/ml/shap_explainer.py`（新規）, `src/database/init_db.py`, `src/pipeline/prediction.py`, `web/src/app/api/shap/[prediction_id]/route.ts`（新規）, `web/src/lib/shapLabels.ts`（新規）, `web/src/components/TodayBuyPanel.tsx`, `tests/test_shap_explainer.py`（新規） |
| 2026-05-27 | 【リアルタイムオッズ自動EV更新】`TodayBuyPanel` にポーリング機能を追加。① `/api/realtime-odds/[race_id]` エンドポイント新設（`realtime_odds` テーブルから馬番ごと最新オッズを返す）。② `TodayBuyPanel` に `raceId` / `pollingIntervalMs`（デフォルト60秒）prop追加、`useEffect` で `setInterval` ポーリング実装。③ ライブオッズが静的 `winOddsMap` を上書き（`effectiveOddsMap = { ...winOddsMap, ...liveOddsMap }`）して Kelly/EV をリアルタイム再計算。④ オッズ変化行にCSSフラッシュアニメーション（`@keyframes odds-flash` 2s）。⑤ 初回フェッチ直後は「オッズ取得中…」、取得完了後はタイムスタンプバッジ「⚡ オッズ更新 HH:MM」を表示。⑥ アンマウント時に `clearInterval` でポーリング停止。`RaceDetail.tsx` から `raceId` を渡すよう変更。Jest 9テスト ALL PASS。影響: `web/src/app/api/realtime-odds/[race_id]/route.ts`（新規）, `web/src/components/TodayBuyPanel.tsx`, `web/src/components/RaceDetail.tsx`, `web/src/app/globals.css`, `web/__tests__/realtimeOdds.test.tsx`（新規） |
| 2026-05-27 | 【発走15分前 Discord アラート】EV >= 1.2 の買い目がある場合、発走14〜16分前に Discord ev_alert チャンネルへ @everyone 付きで通知。メッセージ形式: `🔥 [東京 11R] 発走15分前アラート / 発走予定: 14:30 / EV >= 1.2 の買い目 3件: ・本命 単勝 EV=1.85 推奨投資: ¥5,000 ...`。重複防止: race_id ごと1回のみ通知。環境変数 `PRERACE_ALERT_EV_THRESHOLD` で閾値調整可。影響: `src/notification/prerace_alert.py`（新規）, `src/notification/router.py`, `scripts/scheduler.py` |
| 2026-05-26 | 【当日購入タブ UI 強化（Option B）】`TodayBuyPanel.tsx` を改修。① 結果確定済み行の条件付きスタイリング追加: `is_hit=1`（的中）は薄い赤背景(`rgba(220,80,80,0.15)`)、`is_hit=0`（外れ）は `opacity-40` でグレーダウン。② 確定済み行が存在する場合にのみ表示するサマリーカードを設定パネル上部に新設（合計投資額 / 合計払い戻し額 / 回収率の3KPI、ROI≥100%=緑/ROI<100%=赤）。③ 確定済み行がある場合、テーブルに「結果」列を動的追加（✓ 的中＋払い戻し額 or ✗ 外れを表示）。TypeScript型チェック PASS。影響: `web/src/components/TodayBuyPanel.tsx` |
