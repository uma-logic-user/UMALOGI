...
| 日付 | 変更内容 |
|------|---------|
| 2026-06-01 | 【成果可視化ダッシュボード新設（src/web/dashboard.py）】Streamlit + Plotly で「①直近レース結果（1着馬・単勝オッズ・人気）②本日（最新予想日）のEV上位馬一覧③モデル別ROI/的中率の横棒グラフ（Pure_EV_Edge等・100%損益分岐ライン付き）」を表示するWeb UIを構築。`streamlit run src/web/dashboard.py` で起動。DBは**読み取り専用接続（URI mode=ro）**で開き常駐スケジューラの書込と非競合。データ層を `src/web/dashboard_data.py`（Streamlit非依存の純関数: recent_results / top_ev_horses / latest_prediction_date / model_roi_table）に分離。ROI集計は正準ロジック `src/ml/pnl_accounting.compute_live_roi` を再利用（会計基準の二重定義回避・is_superseded除外・実弾フィルタ）。サイドバーで「実弾のみ/集計開始日」を切替可能。テスト: `tests/test_dashboard_data.py`(9)＋`tests/test_dashboard.py`(6)＋`tests/test_dashboard_app.py`(1=Streamlit AppTest E2E)・全16 PASS。実ヘッドレス起動でHTTP 200/health 200確認。mypy/ruff クリーン。影響: src/web/dashboard.py（新規）, src/web/dashboard_data.py（新規）, src/web/__init__.py（新規）, tests/test_dashboard*.py（新規） |
