...
| 2026-05-23 | 【W-034 新規登録→即完了】バックテストデータリーク修正: `race_results` の 2026 年 DB に rank=1〜3 のみ約 96% が確定済み・rank=NULL が 11,857 件存在。`build_race_df` が `rank IS NOT NULL AND rank > 0` フィルタをテスト時にも適用していたため「実際の 1〜3 着馬のみ」でフィールドを構成→三連複的中率 73.4% (ROI 9294.9%) という虚偽結果を生成。修正: `build_race_df(test_mode=True)` でランクフィルタ除去・entries テーブルを優先使用。`INITIAL_BANKROLL` を ¥10,000 に修正。`kelly_bet` に MIN_BET フロアを追加。修正後の全体 ROI: 84.2%（赤字）、複勝のみ黒字 (ROI 170.8%)。影響: src/analysis/all_bets_backtest_2026.py, .claudecode/rules/honmei_real_bet_rule.md |
