...
| 2026-06-13 | 【W-088 🔴未着手 / 優先度中 — races.grade 列欠落で U score が全予想でスキップ】`src/ml/u_score.py:715` が `SELECT race_id, grade FROM races` を実行するが races に `grade` 列が無く `no such column: grade` で例外→**全レース・全予想で U score 計算がスキップ**（auto_runner.log に毎予想 WARNING 大量出力）。予想生成自体は継続するため件数には影響しないが、前走グレード由来の特徴量が常時欠落し精度劣化要因。対応方針: races に `grade TEXT` 列追加マイグレーション＋JVLink RA レコードからグレード充填（スキーマ変更のため条項2で平日対応）。発見: W-087 調査中の auto_runner.log 解析。影響: src/ml/u_score.py, src/database/schema.py（要migration） |
