...
| 2026-06-15 | 【W-098 🟡研究（v1.16.4-dev） — 内枠複勝バイアスz＋x_trouble_inner三項クロス（両版FAIL・前日版が最有望）】社長指示で `x_trouble_inner`（勝率4/6で最有望だった2項クロス）を三項クロス化。**`src/features/inner_bias.py`**＝内枠(枠1-3)複勝率の z スコアをリークフリー算出（`today_inner_bias`=当日 race_number 小の既走確定のみ／`yesterday_inner_bias`=直近前開催日全体／基準μ,σは全cutoffより前の固定参照 `_BASELINE_REF_HI=2025-10-01`）。**リーク検出の教訓**: テスト作成時、基準(μ,σ)が対象日自身のフル日次率を含むと同日後続レースが基準経由でzに漏れることを `test_inner_bias_w098` が検出→`reference_hi` を対象日より前に限定して根治（研究アセンブラは reference_hi=固定参照を必須）。`research_assemblers.py` レジストリ＋`validate_feature.py --builder/--each` で配線。**6 cutoff ゲート検証**: `x_trouble_inner_today`=勝率33%/平均-5.9pp FAIL、`x_trouble_inner_yest`=勝率50%(3/6)/**平均+3.3pp**/ΔAUC-0.0012 だが勝率2/3未達で FAIL。**前日版>当日版**（当日の既走は標本少で早期バイアス非持続／完了した前日全体が安定信号）。**両版とも本番不採用（条項8）**だが yest が史上最も惜しい（+3.3pp・AUC中立）。次改善: ①EVゲート併用で大穴下振れcutoff除外 ②会場×内外細分 ③直近N開催加重で標本増・勝率一貫性向上。テスト: test_inner_bias_w098 4件。影響: src/features/inner_bias.py, src/features/research_assemblers.py, scripts/validate_feature.py。関連: [[W-097]] [[feedback_no_unvalidated_overlays]] |
| 2026-06-15 | 【W-097 🟢完了（基盤）＋🟡研究中（v1.16.3-dev） — 新特徴量WF検証ゲートの標準化＋前走不利クロス特徴量】W-096の教訓（単一/少数cutoffはノイズで符号反転）を恒久制度化。**Task1（ゲートキーパー・🟢完了）**: `src/ml/feature_gate.py` 新設＝`walk_forward_gate`/`summarize_gate`/`GatePolicy`/`GateResult`。既定PASS=ΔROI改善cutoff>=2/3 ∧ 平均ΔROI>=+2.0pp ∧ 平均ΔAUC悪化<=0.002。CLI `scripts/validate_feature.py`（台帳 `data/feature_gate_results.json`）。**CLAUDE.md 条項8新設**＝新特徴量はゲートPASSなしに本番統合禁止。tests/test_feature_gate_w097.py 8 PASS（prev_trouble_proxy実測-8.15ppが確実にFAILする回帰固定含む）。**Task2（設計・🟢完了）**: `docs/research/cross_feature_ideas.md` に前走不利×既存69列の10案＋上位3（①×内枠`x_trouble_inner` ②×同コース複勝`x_trouble_samecourse` ③×人気妙味`x_trouble_value`）を理由/弱点/改善付きで提示。距離短縮/騎手乗替は要アセンブラ拡張で次イテレーション。**Task3（研究・🟡実行中）**: `scripts/research_cross_features.py` で上位3クロスを実装し6 cutoff WFをバックグラウンド検証中（結果 `data/cross_feature_wf_results.json`・金曜まで）。**いずれもゲートPASS前は本番不採用**（条項8）。影響: src/ml/feature_gate.py, scripts/validate_feature.py, scripts/research_cross_features.py, CLAUDE.md, docs/research/cross_feature_ideas.md, tests/test_feature_gate_w097.py。関連: [[W-096]] [[feedback_no_unvalidated_overlays]] |
| 2026-06-15 | 【W-096 ⚪保留・確定（v1.16.2-dev・6 cutoff ウォークフォワードで決着） — 前走不利プロキシ本番不採用を確定】社長から「第2 cutoffでも有効性確認＝大成果」と評価を受けたが、これは認識違い（第2 cutoff 2025-12 は実測 -4.0pp で反証だった）。決着のため `scripts/walk_forward_trouble.py` 新設で6期間(2025-10〜2026-03)ウォークフォワード検証。**結果: ΔROI 平均 -8.15pp(中央値 -11.9pp)・改善2/6(33%)・ΔAUC平均 -0.0027**。決定打＝当初+5.4ppだった2026-03 cutoffが train/test cap を1800/800→1500/600に変えるだけで **-15.6pp に反転**＝+5.4ppはサンプリングノイズと確定。**さらに概念的誤りも判明**: アブレーションの`BASE`は寄与分離用の6列研究ベースラインで、本番V2(`HonmeiModelV2`)が使う69列`FEATURE_COLS`とは別物→「BASE+trouble(7列)」を正式V2にするのは本番63特徴量を捨てるダウングレード。**確定判定: 本特徴量は本番不採用・コード保持(研究資産)**。本日(月)はJRA開催ゼロ(races=0)のためライブ予想対象なし。教訓: 単一/少数cutoffのOOSは標本ノイズで容易に符号反転する、採否はウォークフォワード平均+勝率で。レポート docs/model_status_report.md 更新。影響: scripts/walk_forward_trouble.py(新)。関連: [[W-096]] [[feedback_no_unvalidated_overlays]] [[feedback_ev_precision_safety_first]] |
| 2026-06-15 | 【W-096 ⚪保留へ更新（v1.16.1-dev・月曜の再学習＋OOS厳格検証の結果） — 前走不利プロキシは本番不採用】Task4 で実装した `prev_trouble_proxy` の本番採否を OOS で厳格検証。`scripts/model_status_report.py` 新設＝BASE / BASE+本特徴量単体 / BASE+全新特徴量 を時系列分割でアブレーション。**単一cutoff(2026-03)では単体+5.4pp(ROI 85.6→91.1%)と有望に見えたが、別cutoff(2025-12)では-4.0pp(107.3→103.3%)で符号反転＝再現性なし(ノイズ)**。全新特徴量セットは両cutoffで一貫悪化(-1.7pp/-22.5pp)。重要度は19列中16位(2.3%)。**判定: 本番投入しない**（条項5「完了は数値改善確認時のみ」・[[feedback_no_unvalidated_overlays]]）。**重要**: ライブV1/V2モデルは base FEATURE_COLS(69列・不変)を使用、`prev_trouble_proxy` は PRERUN/LEAKFREE 経由で研究系(AccuracyModelV2/backtest_v2_oos)のみ → **本特徴量追加は本番推論に無影響＝システムは現行検証済みモデルで安全稼働継続**。本特徴量はコード保持(研究資産)、walk-forward全期間で再現性確認後に再検討。教訓: 単一cutoffのOOSは誤誘導、採否は複数cutoff一貫性で。レポート: docs/model_status_report.md。影響: scripts/model_status_report.py(新)。関連: [[W-096]] [[feedback_no_unvalidated_overlays]] |
| 2026-06-14 | 【W-093 🟢完了（v1.16.0-dev・社長特例で条項2を上書き・Task1） — 週末サイレント停止の根治＋自己修復強化】**真因**: `today_auto_runner.py` のオッズ時系列レコーダー(737行)と x_scraper(782行)が `subprocess.run(stdout=PIPE, text=True, encoding="utf-8")` を **errors 未指定(=strict)** で使用。子(JVLink 32bit由来)が CP932 リードバイト(0x83)を吐くと Python の `_readerthread` が `UnicodeDecodeError` で死亡→**パイプが drain されず子がブロック→300秒タイムアウトを毎周期くり返す空転**（supervisor_stderr.log に大量証拠）。「終了しないハング」のため `UMALOGI_SCHEDULER.bat` の**終了時再起動が発火せず無限に停止**＝6/6に続く自己修復ギャップ([[project_autopilot_no_selfheal_gap]])。**修正**: ①該当2箇所に `errors="replace"` 付与で事故根治。②`src/ops/safe_subprocess.py:safe_run()` 新設＝捕捉 subprocess は errors=replace を強制し事故クラス全体を構造封じ。③`src/ops/heartbeat.py` 新設（原子的鼓動ファイル）＋ `_wait_until`/監視ループ/連続サイクルで `write_heartbeat`、正常終了で `clear_heartbeat`。④`watchdog.py` に `check_autopilot_heartbeat()` 追加＝auto_runner.pid 生存かつ鼓動 stale(>600s) を「ハング」と判定し psutil terminate→supervisor 再起動を誘発（クールダウン300s・Discord通知）。⑤watchdog の `sys.stdout.reconfigure` を try/except で import 安全化。検証: tests/test_self_heal_w093.py 10 PASS（CP932完走/大量drain/鼓動/ハング検知）＋全回帰 1498 PASS。**反映**: コード修正は稼働中オートパイロットの**再起動で有効**。影響: scripts/today_auto_runner.py, scripts/watchdog.py, src/ops/heartbeat.py, src/ops/safe_subprocess.py。関連: [[project_autopilot_no_selfheal_gap]] [[W-085]] |
| 2026-06-14 | 【W-094 🟢完了（v1.16.0-dev・Task2） — 暫定予想の買い目実用化（オッズ未確定でも具体的な印・買い目）】**問題**: 暫定モードでは卍/Oracle が明示スキップ、本命も EV ゲートで全除外され、UI に「印も買い目も出ない」非実用状態。**修正**: `src/ml/provisional_picks.py` 新設＝**オッズ非依存の能力評価（本命勝率スコア）だけ**で ◎〇▲△ の印と具体買い目（単勝/複勝/ワイド/馬連）を生成。`build_output_json` に各馬 `mark`＋`provisional_picks` を同梱。`_prerace_pipeline_inner` で暫定かつ本命買い目が空のとき能力ベース買い目（単勝◎＋複勝◎〇▲）を predictions へ保存（実弾EVゲート非経由）。`web/RaceDetail.tsx` 直前テーブルに能力順「印」列を追加。検証: tests/test_provisional_picks_w094.py 7 PASS。影響: src/ml/provisional_picks.py, src/pipeline/_common.py, src/pipeline/prediction.py, web/src/components/RaceDetail.tsx。 |
| 2026-06-14 | 【W-095 🟢完了（v1.16.0-dev・Task3） — 3連系「本気」アンサンブルモデル】馬連・馬単・三連複・三連単を当てにいく買い目構築エンジンを `src/ml/gachi_trifecta.py` に新設。**アンサンブル方式**＝軸は本命勝率ランク＋複勝率ランク合算が上位の「担保馬」、紐は卍EV上位の「妙味穴馬」。Harville 公式で各順序確率を推定し OddsEstimator で EV 付与、券種別に点数キャップ。payload に `gachi_trifecta` 同梱＋predictions へ `本気3連系(直前/暫定)` で記録（recommended_bet=0＝記録専用・実弾単複ロック外）。オッズ未確定でも確率ベースで動作。検証: tests/test_gachi_trifecta_w095.py 6 PASS。影響: src/ml/gachi_trifecta.py, src/pipeline/prediction.py。関連: [[feedback_ev_precision_safety_first]]（実弾化は単複ロック維持） |
| 2026-06-14 | 【W-096 🟡対応中（v1.16.0-dev・Task4: 特徴量実装は完了／V2再学習＋OOS検証は平日gated） — 前走不利プロキシ特徴量】**新特徴量** `prev_trouble_proxy`＝「速い上がり3F(差し脚)なのに着順凡退・僅差負け」を直線不利/出遅れ/展開不利の代理として[0,1]スコア化（次走巻き返し加点・**リークフリー**＝前走確定結果のみ参照）。`prerun.py` の `PRERUN_FEATURE_COLS` に追加＝`LEAKFREE_NEW_COLS`(backtest_v2)→V2/accuracy_model_v2 入力へ自動合流。**V1 base FEATURE_COLS には含めない**（features.py の V1 build が prerun を結合せず、含めると定数列＋V2で重複するため／入力次元ガード維持）。**既存確認**: 調教(tc_*/hc_*)・レース間隔(days_since_last_race/uf_rest_days)は既にFEATURE_COLSへ統合済。**通過順位**は race_results に列が無く実データ欠如([[W-073]])のため未対応（netkeiba 拡張スクレイプが前提）。**残（gated）**: V2モデルへの活性化＝再学習＋OOS検証が必要（条項5・[[feedback_no_unvalidated_overlays]]「特徴量化→再学習→OOS検証の順厳守」）。週末に live モデルを未検証で上書きしない方針。検証: tests/test_prev_trouble_proxy_w096.py 3 PASS。影響: src/features/prerun.py。関連: [[W-073]] [[feedback_no_unvalidated_overlays]] |
| 2026-06-14 | 【W-092 🟢完了（v1.15.1-dev・社長特例で条項2週末凍結を上書き） — SNS用/個人ガチ用モデルの完全分離・ライブ配線＋判別UI】社長指令「SNSマーケ用多券種と個人実弾用EVモデルを完全分離し、直前バッチで両者をDBへ確実格納＋UIで混同しないよう判別表示」を実装。**ステータス: 🟢完了**（本番反映＝ライブ直前パイプラインに配線済・E2Eはオートパイロットの本日直前バッチで自然発火）。**タスク1（モデル分離・配線）**: W-091 の `_v0525` 多券種を SNS専用(Hit-Focused)、現行単複ロック高EVを個人実弾用(EV-Focused)と定義。`src/pipeline/prediction.py` に best-effort `_maybe_save_shadow()` を新設し `_prerace_pipeline_inner` のライブ保存直後に配線（計算済み df/scores 再利用・別 model_type `_v0525(再計算)` 追記で条項1構造担保・try/except 完全隔離・env `SHADOW_SNS_ENABLE=0` で停止）。`_run_prerace` が毎回新規 subprocess を spawn するため**オートパイロット再起動不要で即時反映**。実弾EVパスは1行も不変。**タスク2（UI）**: `dbHelpers.classifyTrack()`（ハイブリッド: `_v0525`→sns／単複→gachi／多券種→sns）を `/api/predictions` の `track` に付与し `PredictionsPanel` にタブ（全部/💰ガチ/📱SNS）＋行バッジを実装。**タスク3（負荷）**: シャドーは DB 内データのみ・外部スクレイピング追加ゼロで prerace 300s タイムアウトに無影響。検証: pytest 9 PASS（test_prerace_shadow_hook 4＋test_shadow_recompute 5）・jest 32 PASS（classifyTrack 4）・tsc 0・ruff クリーン。影響: src/pipeline/prediction.py, web/src/lib/dbHelpers.ts, web/src/app/api/predictions/route.ts, web/src/types/race.ts, web/src/components/PredictionsPanel.tsx, tests/test_prerace_shadow_hook.py, web/__tests__/classifyTrack.test.ts。関連: [[W-091]] [[feedback_ev_precision_safety_first]] |
| 2026-06-13 | 【W-091 🟢完了（v1.15.0-dev・社長特例で条項2週末凍結を上書き） — 5月末ポリシー復活＝多券種は構造的赤字を実証】社長指令「5月末好調ロジック復活・全券種シグナル復旧」を、live を侵さない**並列シャドー方式**で実装。**「5月末ロジック」の実体を git 走査で確定**＝EV アルゴリズム（Harville+OddsEstimator+本命/卍）は5月末から不変で、差分は**ポリシー層**（2026-06-02 単複ロック=800aa23f の有無）のみ。出典コミット 550ded89（本命三連単EV≥1.5開放）/5e854489（卍三連複ゲート）/02dc564b（ROIフィルター+動的Kelly）。**実装**: `src/analysis/shadow_recompute.py`＝生 `ManjiStrategy`/`HonmeiStrategy`（単複ロック非経由）＋Oracle/HitFocus を直接呼び全7券種を別名 model_id `{base}_v0525(再計算)` で `predictions` へ追記。**条項1を構造的に担保**（シャドーは別 model_type キーで live `卍(直前)` 等と UNIQUE 衝突せず・INSERT OR REPLACE が live を触らない）。`_VALID_BASE_TYPES` に `*_v0525` 7種追加。Task4: 本日36レースを外部通信なし再計算→シャドー624行保存。**実証結果（本日28/36確定）= 仮想ROI 総合7.4%（投資324,300→払戻23,940・収支-300,360）。三連単/馬単/三連複は軒並みROI 0%、単複系も100%割れ。**＝「5月末×全券種で実運用していたら投資の約92%を失う」。**結論: 現行の単複ロックは正しく、多券種復活は value-destroying。シャドー枠は今後の収益トラック監視用に保持**（実弾化は禁止）。検証: pytest 15 PASS（test_shadow_recompute 5＋test_bet_policy 10）・ruff クリーン・DB隔離確認（live 348行不変）。作業前バックアップ: data/backups/umalogi_20260613_234129_pre_shadow_v0525.db。影響: src/analysis/shadow_recompute.py, scripts/recompute_today_shadow.py, src/database/init_db.py, tests/test_shadow_recompute.py。関連: [[feedback_ev_precision_safety_first]] [[feedback_no_unvalidated_overlays]] |
| 2026-06-13 | 【W-088 🟡対応中→平日スクレイピング待ち（v1.14.5-dev・社長承認で条項2解除・前進修正のみ週末安全） — 条件戦 grade 補完の前進修正】**W-088 残課題（条件戦18,745件のgrade空）の精査で前提崩れ2件を発見**: ①`_parse_ra` は確定フィールドのみ保存し**生RAバイト列は未保存**＝「保存済みRAをバイトオフセットで再パース」は対象不在で不可能。②既存RAオフセットは大半が`[推定]`で公式JV-Data仕様と不一致（例: 距離が本コードbyte242・仕様697）。**18,745件の実内訳**: 破損・幽霊行12,362（race_name空/venue文字化け/distance=0/結果0件＝2021-2024失敗インポート残骸）＋文字化け特別/L競走1,671（結果あり・`�`置換で不可逆ロスト）＋真の無名条件戦4,712（結果あり）。**ML価値ある対象は約6,383件のみ**（=結果あり・grade空・status=valid）で全件DB内に決定的ソース無し→netkeiba再取得が必須。**前進修正（本コミット）**: ①`jravan_client.py` `_extract_grade` を全角 `ＧⅠ/Ｇ１・N勝クラス・（L）` 等へ拡充。②`_grade_from_ra(raw)` 新設＝競走条件名称(JyokenName)域 `_RA_JYOKEN_SCAN=slice(27,512)` を cp932 範囲デコードし `_extract_grade` のキーワード照合でクラス導出（**確定オフセット不要・窓ズレ時はキーワード不一致で空文字＝grade誤充填しない安全側設計**。要・実RAサンプルでのオフセット検証）。`_parse_ra` grade を `_extract_grade(race_name) or _grade_from_ra(raw)` のフォールバック配線→**今後の同期で条件戦gradeが自動充填**。③平日バッチ `scripts/fetch_missing_grades.py` 新設（dry-run既定・`--run`/`--sleep`レートリミット・netkeiba二次ソース・predictions不変・冪等）。**残課題（平日実行）**: 月曜深夜に `fetch_missing_grades.py --run` で6,383件を充填予定。**限界**: JVLink側のPattern-1文字化けレースはスキャンも空を返す（netkeiba側で復元）。検証: tests/test_w088_backfill.py 11件（拡充パターン/scan/parse_raフォールバック/W-011 delta/status列）＋既存 test_grade_u_score 9件＋回帰 test_jravan_pipeline/test_features/test_schema 全64 PASS。影響: src/scraper/jravan_client.py, scripts/fetch_missing_grades.py。関連: [[W-089]] [[W-090]] |
| 2026-06-13 | 【W-089 🟢完了（v1.14.5-dev） — 破損・幽霊レース行12,326件の論理隔離】2021-2024の失敗インポート残骸（grade空・distance=0・結果0件・予想0件・date<本日）が races に12,362件残存し特徴量生成/分析クエリのノイズ源。**条項4（物理削除禁止・論理削除優先）に従い物理削除せず論理隔離**: ①`schema.py` races DDL に `status TEXT NOT NULL DEFAULT 'valid'` 追加。②`init_db.py` マイグレーション #22 `_migrate_add_status`（冪等 ALTER）。③`scripts/cleanse_phantom_races.py` 新設（厳格条件＝grade空 AND distance=0 AND date<今日 AND NOT EXISTS結果 AND NOT EXISTS予想・冪等・dry-run対応）で12,326件を `status='error'` に隔離。**条項1完全遵守を検証**: error行のうち予想紐付き=0/結果あり=0/distance>0=0、当日36レースは全 valid 維持。**重要知見**: 「結果なし≠ゴミ」＝当日/未来の未実施レースや予想紐付き行（704件）を誤隔離しないため distance=0＋予想0件＋過去日 を必須条件にした。v_race_mart は INNER JOIN race_results のため結果0件行は元々ML学習に未到達（=隔離は明示的衛生化）。作業前バックアップ: data/backups/umalogi_20260613_140507_pre_w088b.db。影響: src/database/schema.py, src/database/init_db.py, scripts/cleanse_phantom_races.py, tests/test_w088_backfill.py。関連: [[W-088]] |
| 2026-06-13 | 【W-090 🔴未着手 / 優先度中 — U score W-011 クラス変化 delta の符号疑義】`src/ml/u_score.py:_calc_competition` の `class_change_delta = prev_grade_rank - cur_grade_rank`（ランク: G1=1…新馬=10）と docstring「降格馬(強いクラス→)を高評価」が**符号矛盾**の疑い。降格（前走格上=prev_rank小→cur_rank大）では delta が負になり `uf_class_change` が低評価へ寄る＝意図と逆。grade空時は両者ランク5相当で delta=0 縮退（W-088で grade 充填が進めば顕在化）。**週末凍結のためモデル挙動変更は保留**（条項2）。平日に方向性を実データ（降格馬の実成績）で検証し符号を確定すること。tests/test_w088_backfill.py は現状実装（prev-cur）を回帰固定済み。関連: [[W-088]] |
| 2026-06-13 | 【W-087 🟢完了（v1.14.3-dev・社長指令: サーキットブレーカーの Soft Stop 化＋シグナル激減の原因究明） — CB発動時も予想生成を継続】**問題**: 本日11:10にW-043日次損失CB（当日損失¥35,670>¥30,000閾値）が発動し、`_prerace_worker` が `return -2` で **以降27レースの `_run_prerace` を丸ごとスキップ**＝直前予想が9R/暫定30Rの異常状態に（Pure_EVも0件）。データ蓄積・監視が止まる設計上の欠陥。**修正（Soft Stop化）**: ①`today_auto_runner`: `_CB_SOFT_STOP`（env `CIRCUIT_BREAKER_SOFT_STOP`・既定`1`=soft）を新設。soft時はCB発動でも `_run_prerace` を実行し**予想生成・DB保存・監視を継続**、アラートは日次1回に重複排除（旧コードはレース毎にDiscord連投）。hard（`=0`）で旧来のスキップに戻せる。②`src/pipeline/prediction.py:_run_pure_ev_edge`: 同フラグでCB発動時もシグナル生成を継続（旧 `return None` を soft では素通り）。実弾発注は人間判断（安全はアラートと単複ロックで担保）。**シグナル激減の真因＝このCBハードスキップ**（フィルタ閾値ではない）。単複限定ロック（三連系除外）・EV≥1.30/1.15は損失主因排除/エッジ品質の意図的ゲートのため**緩和せず維持**（[[feedback_ev_precision_safety_first]] [[project_pnl_truth_strategy]]）。**復旧実績（BEFORE→AFTER）**: 直前予想 9R→**29R**、Pure_EV 0→**8件**、prediction_horses 916→**1,536件**、premium signals 300/29R→**321/30R**。Web UI `/api/premium-signals`=200。auto_runner再起動でログに `[CB-SOFT] CB発動中だが予想生成を継続` を確認＝本番反映済。テスト: test_pure_ev_wiring 5件（soft/hard両検証）+test_pure_ev_edge 16件+test_bet_generator 36件 PASS。影響: scripts/today_auto_runner.py, src/pipeline/prediction.py, tests/test_pure_ev_wiring.py。関連: [[W-043]] [[W-088]] |
| 2026-06-13 | 【W-088 🟢完了（v1.14.4-dev・社長特例承認で条項2週末凍結を解除し即時改修） — races.grade 列欠落で U score が全予想でスキップしていた問題を根治】**問題**: `src/ml/u_score.py:_calc_competition` が `SELECT race_id, grade FROM races` を発行するが races に `grade` 列が無く `no such column: grade` で例外→`src/ml/features.py:_add_u_score` の try/except が握り潰し**全レース・全予想で U score 18因子+u_score が丸ごとスキップ**（auto_runner.log に毎予想 WARNING 大量出力。最終発生 13:17:28）。**修正**: ①`src/database/schema.py` races DDL に `grade TEXT NOT NULL DEFAULT ''` 追加（新規DB用）。②`src/database/init_db.py` にマイグレーション #21 `_migrate_add_grade` 新設（既存DBへ `ALTER TABLE ... ADD COLUMN`・冪等）。③`src/scraper/jravan_client.py` に `_extract_grade(race_name)` 新設（競走名 [確定] から `(GI)/(GII)/(GIII)/(JGI)/(L)/新馬/未勝利/N勝クラス/オープン` を正準トークン `G1/G2/G3/L/新馬/未勝利/N勝/OP` へ決定的に導出。GradeCDバイトオフセットは未確定[推定]のため不使用）。`_parse_ra` 戻り値と `_save_ra` INSERT へ grade を配線。④`scripts/backfill_grade.py` で既存18,864レースをバックフィル（119件にgrade充填: G1=72/未勝利=23/1勝=13/新馬=7/OP=4）。**列追加はDBレベル変更のため稼働中auto_runnerも次予想から即U score復活**（コード再起動不要。13:17を最後に WARNING 消失）。**残課題**: 条件戦の大半(18,745件)は races.race_name が空でgrade導出不可（JVLink RAの競走条件コードからの充填は別途・バイトオフセット要検証）→ W-011クラス変化はgrade空の馬で delta=0 に縮退（U score本体は正常計算）。検証: 新規 tests/test_grade_u_score.py 9件 + test_jravan_pipeline + test_features + test_schema 全PASS。ライブDBで旧クラッシュクエリ成功確認。影響: src/database/schema.py, src/database/init_db.py, src/scraper/jravan_client.py, scripts/backfill_grade.py, tests/test_grade_u_score.py。関連: [[W-087]] |
| 2026-06-13 | 【W-086 🟢完了（v1.14.2-dev・土曜朝の出馬表未取得/予想未生成SOSで発覚・条項2例外の最小ホットフィックス3件） — 金曜夜間バッチのJVLink同期が構造的に不能だった3連バグ】**バグ①（同期即死）**: `_run_jvlink_sync` が `_jvlink_force_worker.py` を `--fromtime` 無しで呼ぶが、ワーカー側は argparse required → **0秒でrc=2** を返し同期は一度も成功し得なかった（warning止まりで続行＝サイレス失敗。先週まで表面化しなかったのは、W-084で無効化した残骸タスク`UMALOGI_Friday`が別経路で出馬表を取得していたため。残骸停止で初めて単独実走→露呈）。なお`RACE+OPT_TODAY(3)`はJVOpen -501拒否のため fromtime追加だけでは不十分で、失敗時Discordアラートも追加。**バグ②（SE保存全滅）**: `_save_se` の race_results upsert が `ON CONFLICT(race_id, horse_number)` のみで、6/7に導入した部分UNIQUEインデックス `idx_rr_unique_horsenum (WHERE horse_number IS NOT NULL)` と不一致 → SQLiteが「ON CONFLICT clause does not match...」で**新規SE INSERT全滅**（c36ab38f当初からの構造バグ。entries書込も同一try内で道連れ）。conflict targetに `WHERE horse_number IS NOT NULL` を追加して修正。**バグ③（W-080レコーダー恒常死）**: `record_odds_timeseries.py` がsys.pathボイラープレート欠落で `import src` がModuleNotFoundError → 6/11配線以降一度も走れていなかった。_ROOT挿入で修正（修正後オッズ時系列544スナップショット蓄積開始を確認）。**復旧手順**: 実績経路`data_sync.sync_friday`(NORMAL→STORED→SETUP)を32bitで手動実行→races土日各36R入庫→netkeiba `refetch_entries_from_netkeiba.py`で出馬表補完→`force_provisional_today.py`で暫定予想31R/124件→`premium_pack`6ファイル生成→auto_runner再起動（スーパーバイザーself-heal経由で修正コード反映・09:13土曜監視ループ突入）。Web UI `/api/premium-signals`=200(29R/300シグナル)。**残課題**: 日曜分はnetkeiba側未公開のため土曜夜間バッチ（修正済みコード）で取得予定。netkeibaパーサーが一部レースで「tr.HorseList行はあるが0頭」を返す事象は要監視。影響: scripts/today_auto_runner.py, src/scraper/jravan_client.py, scripts/record_odds_timeseries.py。関連: [[W-084]] [[W-080]] |
| 2026-06-12 | 【W-085 🟢完了（v1.14.1-dev） — ログオン時自動復旧の死角2件: Web UI(3000)不在＋bat日本語CP932誤パース（PC再起動後にWeb UI停止のまま、で発覚）】**死角①**: 自動復旧経路（`UMALOGI_Startup`タスク→`startup_umalogi.bat`→`scripts/bat/start_umalogi.bat`）の起動対象が Streamlit/auto_runner/watchdog の3つのみで、**Next.js Web UI(ポート3000)がどの自動起動経路にも含まれていなかった**。**死角②（より深刻・恒常障害の真因）**: UTF-8保存の日本語入り bat は新規コンソール（初期CP932）で cmd.exe に誤パースされる — 全角文字のUTF-8末尾バイト(0x81-0x9F域)がCP932先行バイトと解釈され、直後の `%`・引用符・**改行(CR)**まで食われ、変数未展開（`%SCHED_RUNNING%`が文字列のまま）・行結合・誤分岐が発生。実害として(a)正規ランチャーが「scheduler.py稼働中」と誤判定し`pause`で永久停止＝**今朝8:31のログオン自動復旧がStreamlitすら起動できていなかった**、(b)`start`行の誤解釈で秀丸が`today_auto_runner.py`を開く誤動作。bat内`chcp 65001`では防げないことを実証。**死角③**: スタートアップフォルダ`UMALOGI 起動.lnk`→ルート`start_umalogi.bat`（旧世代）が**排他禁止のscheduler.pyを直接起動**する残骸で、ログオン毎に二重自動運転リスク。**対処（全て完了・E2E検証済）**: ①正本`scripts/bat/start_umalogi.bat`に「4) Next.js Web UI」を追加（`:countport`=ポート3000 LISTEN判定ガード・`.next`無ければ`npm run build`・`UMALOGI_WEBUI`ウィンドウ）。`startup_umalogi.bat`のフォールバックにも追加。②ランチャー4ファイル（start/stop/startup/ルートシム）を**100% ASCII化**（日本語はREMからも排除・ルールをREADME_BAT.mdに明文化）。③ルート`start_umalogi.bat`をscheduler.py起動を廃した正本への委譲シムに書換（lnk互換維持・二重運転経路遮断）。④`stop_umalogi.bat`にポート3000のnode限定停止を追加。**検証**: ASCII版を新規コンソールでE2E実行→ガード正常動作（autopilot/watchdogスキップ・Streamlit/Web UI起動）→ **http://localhost:3000 = HTTP 200 / 8501 = HTTP 200**。次回再起動時はタスク+lnk+vbsの全経路が同一の正本ランチャーに収束。**追加発見（同日・同根）**: W-084で導入した`UMALOGI_SCHEDULER.bat`スーパーバイザー（UTF-8日本語入り）も同じCP932誤パースの罠を踏んでおり、**初回起動のみ正常・`goto loop`での再走査時に行が崩れ、再起動リトライが全て即死するクラッシュループ**に陥っていた（今朝08:28にauto_runner初回インスタンス死亡後、13:20まで約5時間オートパイロット不在。ロックファイルは更新されず子はpy.exe出現→数秒で消滅を繰り返し）。対処: スーパーバイザーも100% ASCII化＋子のstderrを`data/supervisor_stderr.log`へ追記捕捉（file logging確立前の早期クラッシュを永続化）。検証: 新スーパーバイザー配下で子(lock正常取得・ゾンビPID自動クリア実動)が安定稼働し`待機中`ログ再開・金曜20:00バッチ待機を確認。影響: scripts/bat/start_umalogi.bat, scripts/bat/stop_umalogi.bat, startup_umalogi.bat, start_umalogi.bat, UMALOGI_SCHEDULER.bat, scripts/bat/README_BAT.md。関連: [[W-084]] |
| 2026-06-12 | 【W-084 🟡対応中 / 優先度高 — タスクスケジューラ残骸による二重自動運転・無監督常駐リスク（金曜20:00直前のインフラ監査で発見）】①`UMALOGI_Friday`（毎金20:00・先週6/5も実行・結果0）が `run_friday_batch.bat` を起動し、オートパイロット内蔵の金曜夜間バッチ（`_run_friday_batch`）と**今夜20:00に二重実行衝突**する構図だった（JVLink競合・予想二重INSERT・Discord二重通知・8GB RAM逼迫下の二重LightGBM=OOMリスク）。②`UMALOGI-Scheduler`（ログオン時）/`UMALOGI_Scheduler`（ブート時）が排他禁止の `scheduler.py` を起動する設定で有効のまま残存。③稼働中の auto_runner/watchdog は親プロセス消滅済の「裸」状態で、死亡時に誰も再起動しない（6/6オートパイロット停止障害の再発条件）。**応急対処（同日実施・コード変更なし）**: (a) 重複watchdog(PID 28184)をkillし、`UMALOGI_Watchdog` タスク（5分毎再生成=self-heal）を watchdog の単独オーナーに一本化 (b) auto_runner を `UMALOGI_SCHEDULER.bat`（30秒リトライ・PIDロックガードで重複安全）配下に移行し self-heal 化（新PID 55580・20:00待機確認済） (c) `run_friday_batch.bat` → `run_friday_batch.bat.DISABLED_20260612` リネームで今夜の衝突経路を遮断（`git restore run_friday_batch.bat` で復元可）。**残作業（管理者権限必須・ユーザー実行待ち）**: 管理者PowerShellで `Disable-ScheduledTask UMALOGI_Friday / UMALOGI-Scheduler / UMALOGI_Scheduler` → bat を git restore して正規フォールバックとして再設計（W-085候補: bat先頭にオートパイロット排他ガード追加）。さらに **Windows Update 再起動保留フラグが2つともTRUE** — 週末中の自動再起動=サイレント全停止の最有力シナリオのため、20時前の手動再起動（＝ログオン時自動復旧経路のE2Eテストを兼ねる）を推奨。影響: 運用環境のみ。関連: [[W-083]] CLAUDE.md 本番稼働アーキテクチャ節 |
| 2026-06-12 | 【W-083 🟢完了（同日修正・v1.14.0-dev） — TARGET frontier JV 本体への IDOK 無限連打を解消】真因は Delphi 製 TARGET の不可視アプリケーションウィンドウ（クラス `TApplication`・タスクバー用でダイアログではない）がタイトルパターン `target frontier` に一致していたこと。`_EXCLUDED_WIN_CLASSES` に `tapplication` を追加して除外（TARGET の実ダイアログは `#32770`/TForm 系のため突破機能は不変）。同時に、Claude Code セッション等の**ターミナルウィンドウのタイトル**（「〜セットアップ」等の作業内容表示）が `セットアップ` パターンに誤一致して IDOK が連打される新発見の同型問題も `consolewindowclass`/`cascadia_hosting_window_class`/`vscodemainwindow` 除外で根治。E2E検証: 再起動後の auto_runner.log で両連打の完全停止を確認・test_jvlink_dialog_handler 39 PASS。残課題（任意強化・低優先）: 同一 hwnd への送信回数上限は未実装。 |
| 2026-06-12 | 【W-083 🔴未着手 / 優先度中 — jvlink_dialog_handler が常駐アプリ「TARGET frontier JV」のメインウィンドウへ IDOK を無限連打】e62df054 ドライラン検証中に発見。タイトルパターン `target frontier` が**ダイアログではなくTARGETアプリ本体のメインウィンドウ**（hwnd=15600928・閉じない）に一致し、クールダウン1.5秒毎に `WM_COMMAND IDOK` を永久送信。影響: ①auto_runner.log が DialogHandler 行で異常肥大（15,511行中の大半・障害解析時に本質ログが埋没） ②ユーザーが TARGET frontier JV を起動中の場合、アプリへ意図しない OK コマンドが流れ続け操作を阻害するリスク。対応方針: ①ウィンドウクラス名 `#32770`（標準ダイアログ）への限定判定 ②同一 hwnd への送信回数上限（N回失敗で諦めて1回だけ WARNING、以後抑制） ③`target frontier` パターンの再考（TARGET本体は突破対象のセットアップダイアログではない）。担当フェーズ: 来週平日（条項2: 週末は変更禁止）。影響ファイル: src/ops/jvlink_dialog_handler.py。関連: [[W-082]] docs/6_special_notes.md 2026-06-12 エントリ |
| 2026-06-11 | 【W-082 🔴未着手 / 優先度低 — UI/UX強化(v1.13.0-dev)で発生した軽微な未配線】UI/UX 超絶強化（CUI Rich化/Discord プレミアムEmbed/HTML レポート）は完了したが、以下 2 点が「API実装済・実値未配線」: ①`notify_prerace_result(confidence=)` の自信度グラデーション色はパラメータのみで、prediction パイプラインからモデル自信度の実値を渡していない（現状は格付け色→既定シアンで動作・機能劣化なし） ②投資比率バーの分母は `UMALOGI_BANKROLL` 環境変数（既定10万円）の静的値であり、`bankroll_manager.effective_bankroll`（確定損益からの動的算出）と未接続。また RichHandler コンソール装飾は today_auto_runner のみで watchdog.py は plain のまま（意図的・低リスク優先）。対応方針: W-078 の bankroll_manager 本番結線時に bankroll を、モデル較正済確率の代表値が定義できた時点で confidence を配線。影響(将来): src/pipeline/prediction.py, src/notification/router.py。関連: [[W-078]] docs/ui_ux_upgrade_report.md |
| 2026-06-11 | 【W-081 🟢完了 / 優先度中 — 当日追記5】**過去モデル資産の未活用（リポジトリに眠る学習済みpklがEVエンジンに不参加）を解消**（v1.12.0-dev）。全保存pkl（ALPHA/ALPHA-Payout/cascade stage2-3/sandbox 3種/honmei・manji・place v2系/pre69feat世代）を `scripts/analyze_legacy_models.py` で OOS 400R 静的解析。**採用=卍(現役EV回帰)のみ**: 荒れレースAUC 0.754(市場0.612超)×ρ(honmei)=0.33の真の多様性。`src/ml/legacy_ensemble.py` の `ensemble_win_probs`（総和保存・w=0恒等・失敗時フォールバック）で honmei 勝率に w=0.4 融合し、**三連複限定**で premium_pack に本番配線（今週末オートパイロットから自動有効）。**完了の定義充足**: OOS 400R 実払戻清算で合計ROI 110.0%→**119.2%**・三連複106.9%→**157.9%**・最大1的中除外81.8%→**107.8%** を数値実証＋全テストPASS。不採用理由も記録: ALPHA系=ρ(market)≈0.96-0.98の市場複製/cascade=stage1 pkl未保存で入力再現不能/sandbox・pre69feat・v2系=AUC不足。**新規負債なし**（実弾ポリシー単複ロック不変・三連系は購読コンテンツのみ）。残課題（保留・低）: 卍w=0.4はtrainフレーム探索値であり、四半期ごとの再学習時に backtest_all_tickets.py --frame train での重み再探索を推奨。影響: src/ml/legacy_ensemble.py(新規), src/marketing/premium_pack.py, scripts/analyze_legacy_models.py(新規), scripts/backtest_all_tickets.py, tests/test_legacy_ensemble.py(16件)。関連: [[W-080]] [[feedback_no_unvalidated_overlays]] |
| 2026-06-11 | 【W-078 🟡対応中(シミュレーション機能完成・本番結線は引き続きOOSゲート待ち) / 優先度中 — 当日追記4】**三連系ハイボラ環境向けポートフォリオ破産シミュレーションを実装し「破産確率1%未満の最適ベット額」を機械算出可能にした**。`simulate_portfolio_ruin`(同一レース内排反・レース間独立・対数複利MC 4000パス×300ラウンド)＋`recommend_portfolio_stakes`(P(破産)≤target_ruinの最大Kelly分数探索)。実証: 三連系5点(EV2.0-2.8)で1/10 Kelly=P(破産)0.0%(中央値8.5倍)・分数0.20=P(破産)0.83%<1%・フルKelly=P(破産)40.5%。サブスク向け premium_pack の参考ベット額に allocate_stakes を結線済（実弾は未結線=bet_policy単複ロック不変）。残: 実弾解禁時の「現行固定Kelly vs 動的管理」OOS比較。影響: src/ml/bankroll_manager.py, src/marketing/premium_pack.py, tests/test_bankroll_manager.py(32件) |
| 2026-06-11 | 【W-080 🟡対応中(本番配線済・実蓄積は次開催実証待ち) / 優先度高 — 当日追記3】**オッズ時系列レコーダーが本番オートパイロットに未配線＝スマートマネー特徴量の歴史データが永遠に蓄積されない構造だった**。発覚経緯: 「朝→直前オッズ変動率（異常投票検知）」特徴量の検証で `odds_timeseries`=**0行**・`realtime_odds`=48レース実質単一時点・`multi_odds`=1レース分のみと判明。`scripts/record_odds_timeseries.py`（P0-1で実装済）は**不使用の scheduler.py の10分ジョブにのみ配線**され、本番の `today_auto_runner --continuous` からは一度も呼ばれていなかった（W-064/065と同型の「実装済み・未配線」サイレントギャップ）。**対応**: `_run_odds_timeseries_recorder()` を新設し監視ループ(`_run_one_day`)から**約10分間隔**で subprocess 起動（5分timeout・失敗で監視ループを止めない・`ODDS_TIMESERIES_DISABLED=1`で無効化可）。**代替プロキシの検証と棄却**: 時系列が無い間の代替として Shin(1993) z値＋shin_uplift を400レース/5,588馬行で検証（`scripts/validate_smart_money.py`）→ **uplift上位10%は559点で的中0件=ROI 0%・単独エッジなしで棄却**（upliftは実質オッズ変換でFLB補正済み市場に追加情報なし）。**残**: ①次開催（土曜）で realtime_odds への複数スナップショット蓄積を実証 ②数週間の蓄積後に「変動率」特徴量を構築しOOS検証。影響: scripts/today_auto_runner.py(配線), scripts/validate_smart_money.py(新規)。関連: [[W-064]] [[W-065]] |
| 2026-06-11 | 【リーク監査完遂 🟢 — ハイブリッドアンサンブルHOLD→**棄却**確定（v1.9.1-dev・当日追記2）】Ablation監査(`scripts/ablation_leak_audit.py`)で真因特定: `build_race_features` の `_get_horse_stats_bulk` 呼び出しに `exclude_race_id`/`race_date` 欠落→歴史レースで**当該・未来レースの着順がwin_rate系に混入**（`win_rate_distance_band_zscore` 単独AUC 0.957）。features.py修正＋リーク回帰テスト化。**修正後の真値**: AccuracyModelV2 AUC 0.951→**0.664**・二階層アンサンブルROI 1173%→**76.0%**(EV単体92.5%に劣後)＝**昇格棄却**。本番モデルはsimulate経路(リーク防御済)で学習のため無傷。W-071鉄則が3例目のリークを捕捉（加速力W-070→win_rate今回）。詳細: docs/leak_audit_and_integration_report.md。影響: src/ml/features.py, tests/test_features.py, scripts/ablation_leak_audit.py(新規), scripts/e2e_final_prediction.py(新規・市場ブレンド適用E2E)。関連: [[W-070]] [[W-071]] |
| 2026-06-11 | 【W-079 ⚪保留(有効性検証で仮説棄却・シャドー昇格中止) / 優先度低 — 当日追記】**実弾確定P&L 549件/438レースへの遡及検証で「カオス見送り」仮説が棄却された**。`scripts/validate_no_bet_filter.py`(新規)で chaos閾値をスイープ: 既定0.42=発火1/438レース(0.2%)でROI 91.9→92.0%(+0.1pt・実質中立)、**閾値0.15〜0.30では-8.1〜-8.9ptの逆効果**（見送りレースが勝ち越し）。**学び: 現行実弾モデル（卍複勝/Pure_EV）は混戦レースの市場の歪みこそが利益源**であり、混戦回避は利益源を切る行為だった。シャドー運用昇格は中止し、モジュールは①オーバーラウンド異常（データ供給異常）検知 ②E2E出力の「見送り表示」演出（サブスク向け規律の見せ方）用途に限定して存続。再評価条件: 実弾ミックスが的中率特化型へ変わった場合のみ。影響: scripts/validate_no_bet_filter.py(新規)。関連: [[feedback_ev_precision_safety_first]] |
| 2026-06-11 | 【W-079 🟡対応中(モジュール実装完了・シャドー運用は次フェーズ) / 優先度中】**見送り(No-Bet)判定モデルの実装と段階導入計画**。`src/ml/no_bet_filter.py` 新規: レース単位 chaos_score（オッズエントロピー0.30/弱い本命0.20/オーバーラウンド異常0.20/モデル-市場JS乖離0.20/構造リスク0.10）≥0.42 で見送り判定。確率・EVへの係数操作は構造的に不可能なAPI（W-071遵守・テストで担保）。テスト14件PASS。**残（🟡の理由）**: ①Phase2=直前パイプラインでのシャドー運用（判定をnotesに記録のみ・実弾不変）②見送りレースの実ROI集計で「見送り正解率」を実測 ③改善実証後にPhase3実弾ゲート昇格。閾値0.42・重みは初期設計値でありシャドー実測で再較正する。影響: src/ml/no_bet_filter.py(新規), tests/test_no_bet_filter.py(新規)。関連: [[W-071]] [[W-078]] docs/fable_ultimate_upgrade.md |
| 2026-06-11 | 【残タスクスイープ 🟢完了】**worktree隔離資産のmaster移植によるorphan解消とハイブリッドアンサンブル検証実行**。①`src/ml/accuracy_model_v2.py` をaccuracy-model worktreeからmasterへ移植し、import不能だった `tests/test_accuracy_model_v2.py`(6件) をPASS化（前セッションで発見した並行セッション残骸の正式解決）。②`scripts/evaluate_hybrid_ensemble.py`（EV×Accuracy二階層アンサンブルOOS検証・保留タスク）を移植。**移植時に実バグ発見・修正**: honmei Boosterへの入力がDataFrame列存在フィルタで40列に欠け `LightGBMError: 40 != 69` でクラッシュ→`reindex(columns=FEATURE_COLS).fillna(0)`で69列整列。実DBでOOS検証を実行: 二階層アンサンブル(L1∩L2) 35ベット・的中65.7%・ROI1173% vs Accuracy単体629.7%/EV単体89.2%。**ただし昇格判定はHOLD**＝的中率72.8%/AUC0.944は市場情報からは原理的に説明困難な「良すぎる数値」(W-071鉄則)であり、小標本(35件)+最大142.7倍大穴+確定オッズ由来特徴量のタイミングリーク疑い+val閾値の分布近接楽観のリーク監査(特徴量ablation)が本番採用の前提（詳細: docs/fable_ultimate_upgrade.md §タスク2）。③`legacy_bridge.py`はリポジトリに実在せず（bet_policy.base_model()が世代ブリッジを担当済・追加実装不要と判断）。④src/・scripts/のTODO/FIXMEコメントは0件。影響: src/ml/accuracy_model_v2.py(移植), scripts/evaluate_hybrid_ensemble.py(移植+修正)。関連: [[W-070]] project_business_4domains_20260611 |
| 2026-06-11 | 【W-078 🟡対応中(モジュール実装完了・本番結線はOOSゲート待ち) / 優先度中】**バンクロール管理の数学的穴4点（同時ベット合成Kelly無視/静的バンクロール/破産確率未定量化/ドローダウン無減速）を解消するモジュールを実装、ただし本番未結線**。`src/ml/bankroll_manager.py` 新設: `allocate_stakes`(1点2%＋日次合計10%比例縮約)・`effective_bankroll`(確定損益から現在資金/ピーク動的算出)・`drawdown_throttle`(-10%/-20%/-30%で半減/1/4/全停止)・`estimate_ruin_probability`(ベクトル化モンテカルロ)・`recommend_kelly_fraction`(P(破産)≤目標の最大分数)。テスト25件PASS・mypy0。**残（🟡の理由）**: 本番ベットフロー(prediction.py/pure_ev_edge)への結線は「現行固定1/10 Kelly vs 動的管理」のOOSバックテスト比較（最大ドローダウン・破産確率・最終資産の3指標）で優位を確認してから（[[feedback_ev_precision_safety_first]]・W-071の教訓）。EVロジック精査で特定した残穴も台帳化: 複勝オッズ線形近似の系統誤差（JVLinkリアルタイム複勝オッズ取得が根治策・🔴未着手）/オッズ取得タイミング乖離の定量化（🔴未着手・卍単勝OOS<100%の真因候補）→ docs/business_architecture_fable.md §1.2 参照。影響: src/ml/bankroll_manager.py(新規), tests/test_bankroll_manager.py(新規), config/automation_daily.yaml(新規), docs/business_architecture_fable.md(新規)。関連: [[W-066]] [[W-071]] |
