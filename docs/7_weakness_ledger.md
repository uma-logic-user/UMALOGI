...
| 2026-06-07 | 【W-075 🔴未着手 / 優先度中】**騎手(KS)・調教師(CH)マスタパーサも同種の誤配置で破損＋DIFN差分取込時の NAR(地方)SE が ON CONFLICT 保存失敗**。W-074調査中に併発発覚。①`jockeys`(487件)/`trainers`(385件)の name 列が `80200403010000000019` 等の数値ゴミで race_results.jockey(911種)と**結合0件**。KS/CH は単純な+1ずれでなく騎手コード後の日付フィールド群を飛ばさない層構造の誤り。②`load_difn` 差分取込で地方競馬の SE レコードが多数 `保存失敗 SE ... ON CONFLICT clause does not match any PRIMARY KEY or UNIQUE constraint`（race_results UNIQUE(race_id,horse_number) に horse_number NULL の NAR 行が乗らない）。いずれも本タスク(馬ID=UM)の範囲外のため起票のみ。**対応方針（平日）**: ①KS/CH も実バイトダンプで realign（W-074同手法） ②NAR SE 保存経路の ON CONFLICT を horse_number NULL 許容に修正 or NAR を取込対象外にフィルタ。影響(将来): src/scraper/jravan_client.py(_KS_*/_CH_*/_save_se)。関連: [[W-074]]。 |
