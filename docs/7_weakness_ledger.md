...
| 日付 | 更新内容 |
|------|---------|
| 2026-06-08 | 【W-077 🟡対応中(再学習・OOS完了/聖域反映は別途) / 優先度高】**2025年以降クリーンデータで全モデル再学習＋カンニングなしOOS再シミュレーション**（v1.7.0-dev）。W-074/075/076 の血統修復・騎手調教師コード結合の恩恵を取り込んだ「超高品質1.5年(2025-2026)」で本命/複勝/卍を再学習。**歴史データ(2024以前)はJVLink SID制約で取得不能を3方式実証(NORMAL=保持期間外/STORED=-501/SETUP=-503)→2025+を正とする方針確定**。実装: `_build_train_df`/`train_all`/`train()` に `train_from` 下限フィルタ＋特徴量1回生成→3モデル共有の最適化(約3倍速)。`backtest_all_models.py` に `--train-year/--test-year/--single-year-train` 追加。**再学習結果**: 本命CV AUC 0.7191/複勝0.7302、特徴量重要度 **Top5 に jockey_code_encoded(3位)・trainer_code_encoded(5位)** が昇格(コード化が主力エッジ化を実証)。**OOS(2025学習→2026テスト・cardboard-free)**: 本命単勝617.7%/卍単勝495.8%/ALPHA単勝424.2%(⚠️高分散・少数大穴依存)、安定黒字=複勝Top3流し110.4%・複勝Top1 102.1%・本命三連複112.6%。**残**: ①日中backfillで2025前半コード化(現56%) ②2026会場別OOSで `_FUKUSHO_ELITE_VENUES`(聖域)再定義 ③高ROI単勝の分散リスク評価後に実弾判断([[feedback_ev_precision_safety_first]])。影響: src/ml/models.py, src/ml/features.py, src/database/init_db.py, src/database/schema.py, scripts/retrain_win_place.py, scripts/backtest_all_models.py, scripts/fetch_3years_history.py(新規), data/models/*.pkl。関連: [[W-076]] project_alpha_model。 |
