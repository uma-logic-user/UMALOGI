...
| 日付 | 変更内容 |
|------|---------|
| 2026-06-14 | 【暫定買い目実用化＋3連系本気モデル＋前走不利特徴量（W-094/095/096・v1.16.0-dev・社長特例で条項2を上書き）】**Task2(W-094)**: 暫定予想がオッズ未確定で買い目を出せなかった問題を、`src/ml/provisional_picks.py` 新設で解決＝本命勝率スコアのみ（オッズ非依存）で ◎〇▲△ の印と具体買い目（単勝/複勝/ワイド/馬連）を生成。`build_output_json` に各馬 `mark`＋`provisional_picks` を同梱し、暫定で本命買い目が空のとき能力ベース買い目（単勝◎＋複勝◎〇▲）を predictions へ保存。UI `RaceDetail` 直前テーブルに能力順「印」列を追加。**Task3(W-095)**: `src/ml/gachi_trifecta.py` 新設＝3連系本気アンサンブル。軸＝本命勝率ランク＋複勝率ランク合算の上位「担保馬」、紐＝卍EV上位の「妙味穴馬」。Harville＋OddsEstimator で馬連・馬単・三連複・三連単を構築し payload `gachi_trifecta`＋predictions（`本気3連系(直前/暫定)`・記録専用・実弾単複ロック外）へ。**Task4(W-096)**: 前走不利プロキシ `prev_trouble_proxy`（速い上がりで凡退＝展開不利の巻き返し・リークフリー）を `prerun.py` に実装し `PRERUN_FEATURE_COLS→LEAKFREE_NEW_COLS`（V2/accuracy 入力）へ合流（V1 base FEATURE_COLS には非追加＝入力次元不変・V2重複回避）。V2活性化の再学習＋OOS検証は平日gated（[[feedback_no_unvalidated_overlays]]）。検証: pytest 1498 PASS（新規16件）。影響: src/ml/provisional_picks.py(新), src/ml/gachi_trifecta.py(新), src/features/prerun.py, src/pipeline/_common.py, src/pipeline/prediction.py, web/src/components/RaceDetail.tsx。関連: [[W-094]] [[W-095]] [[W-096]] |
| 2026-06-14 | 【SNS用／個人ガチ用モデルの完全分離・ライブ配線（W-092・v1.15.1-dev・社長特例で条項2週末凍結を上書き）】W-091 の `_v0525` 多券種シャドーを **SNS・マーケティング専用（Hit-Focused）**、現行単複ロック高EVを **個人実弾投資用（EV-Focused）** と役割定義。これまで `_v0525` は手動 `recompute_today_shadow.py` 実行分しか `predictions` に存在しなかったが、**本日の直前予想ライブバッチで自動生成**されるよう配線。実装＝`src/pipeline/prediction.py` に best-effort ヘルパー `_maybe_save_shadow(conn, race_id, df, honmei_scores, ev_scores)` を新設し、`_prerace_pipeline_inner` の**ライブ保存（`_save_predictions`）直後**に呼ぶ。**計算済みの `df/honmei_scores/ev_scores` を再利用**するため特徴量の二重生成も外部スクレイピングも追加せず、`generate_shadow_bets`→`save_shadow_bets` で `{base}_v0525(再計算)` を追記（条項1: 別 model_type のため既存 live 予想を構造上 UPDATE しない）。**実弾EVパス（`_save_predictions`/Pure_EV_Edge/FukushoElite/単複ロック）は1行も変更せず**、シャドーは try/except で完全隔離（例外は warning ログのみ・再送出せず実弾予想と Discord 通知を絶対に止めない）。env `SHADOW_SNS_ENABLE=0` で緊急停止可。**オートパイロット再起動不要**＝`_run_prerace` が毎回 `py -m src.main_pipeline prerace` を新規 subprocess で spawn するため src 変更が即時反映。検証: pytest 9 PASS（test_prerace_shadow_hook 4＝保存/env無効化/best-effort/live不変＋test_shadow_recompute 5）・ruff クリーン。影響: src/pipeline/prediction.py, tests/test_prerace_shadow_hook.py。関連: [[W-091]] [[W-092]] [[feedback_ev_precision_safety_first]] |
| 2026-06-13 | 【5月末ポリシー並列シャドー再計算＝多券種復活は構造的赤字を実証（W-091・v1.15.0-dev・社長特例で条項2週末凍結を上書き）】社長指令「5月末好調ロジック復活・全券種シグナル完全復旧」を、**live を一切侵さない並列シャドー方式**で実装。**「5月末ロジック」の実体**＝EV算出（Harville+OddsEstimator+本命/卍モデル）は5月末から不変で、消えた馬連・馬単・ワイド・三連複・三連単は**ポリシー層**（2026-06-02 の単複ロック=`is_live_bet`）で出力除外されていただけ（バグではない）。`src/analysis/shadow_recompute.py` 新設＝生 `ManjiStrategy`/`HonmeiStrategy`（単複ロック非経由）＋`VirtualOracleStrategy`/`HitFocusStrategy` を直接呼び全7券種を別名 model_id `{base}_v0525(再計算)` で `predictions` へ**追記のみ**（既存 live 予想は別キーで不変＝条項1構造担保）。`scripts/recompute_today_shadow.py` で本日36レースを外部通信なし再計算→624行保存→仮想ROI出力。**実証（本日28/36確定）= 仮想ROI総合7.4%（投資324,300→払戻23,940・収支-300,360）。三連単/馬単/三連複は軒並みROI 0%、単複系も100%割れ＝「5月末×全券種で実運用なら投資の約92%喪失」。結論: 現行の単複ロックは正しく、多券種復活は実弾化禁止（シャドーは収益トラック監視用に保持）。** 影響: src/analysis/shadow_recompute.py(新規), scripts/recompute_today_shadow.py(新規), src/database/init_db.py(_VALID_BASE_TYPES に *_v0525 追加), tests/test_shadow_recompute.py(新規5件)。関連: [[W-091]] [[feedback_ev_precision_safety_first]] |
