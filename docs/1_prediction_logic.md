...
| 日付 | 変更内容 |
|------|---------|
| 2026-05-24 | 【ROIフィルター + 動的Kelly実装】本番実績ROI（2026-04〜05データ）分析結果を反映。`_ALLOWED_BET_TYPES` で本命モデルの三連単(ROI66%)/馬単(52%)/馬連(48%)/ワイド(41%)を除外、単勝/複勝のみ許可。`get_dynamic_kelly_fraction()` 新設（ROI300%超=1/4Kelly、ROI200-300%=1/5Kelly、ROI100-200%=1/10Kelly、ROI<100%=購入禁止）。`get_model_bet_roi()` でDB照会ベースの ROI 算出関数追加。`BetGenerator._apply_roi_filter()` が `generate_honmei()` `generate_alpha_trifecta()` を自動フィルタリング。シミュレーション効果: ROI74.3%→163.4%（損益-¥150万→+¥67万）。影響: `src/ml/bet_generator.py` |
