...
| 日付 | 内容 |
|------|------|
| 2026-06-11 | 【SNS集客・サブスク導線の日次自動生成エンジン新設（v1.8.0-dev）】`src/marketing/sns_generator.py` を新規実装。**盾と矛戦略**＝無料公開は勝率特化（本命系）上位3レースのみ（盾・信頼獲得）、高EV穴馬・買い目・推奨ベット額は有料側に温存（矛・課金価値）。生成物3点を outputs/marketing/YYYYMMDD/ へ日次出力: ①free_pick.md(無料予想ポスト・文字化け/空馬名除外=CLAUDE.md§16遵守・信頼度はconfidence→model_scoreフォールバックで「AI勝率予測N%」表示) ②hit_report.md(前日確定実績・**bet_policy.is_live_bet で実弾のみ集計**=観賞用混入による数字の不誠実を構造的に排除・真コスト基準ROI・負け日も誠実公開) ③video_script.md(CapCut向けシーン分割台本=映像指示/ナレーション/テロップ)。全生成文に `KILLER_PHRASES`(6種・日付決定的ローテーション)でサブスク導線を論理的に埋め込み（認知転換/大数の法則/投資アナロジー/リスク管理/限定性/データ権威の6型）。自動投稿は当面オフ(auto_post: false・X API規約確認後に解禁判断)。CLI: `py -m src.marketing.sns_generator --date YYYYMMDD`（タスクスケジューラ UMALOGI_DailyMarketing が毎日21:00起動）。実DB検証: 2026-06-07で実弾ROI109%・実馬名・的中5/17を正しく生成。テスト tests/test_sns_generator.py 17件PASS。ファネル全体設計は docs/business_architecture_fable.md 領域3/4参照。影響ファイル: src/marketing/sns_generator.py(新規), src/marketing/__init__.py(新規), scripts/bat/register_daily_tasks.ps1(新規) |
