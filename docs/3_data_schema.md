...
| 2026-06-13 | 【races.status 列追加 — 破損・幽霊行の論理隔離（W-089・v1.14.5-dev・条項4論理削除）】migration #22 `_migrate_add_status` で `races.status TEXT NOT NULL DEFAULT 'valid'` を additive 追加（語彙: `valid`=正常 / `error`=破損隔離）。2021-2024の失敗インポート残骸（race_name空・distance=0・結果0件・予想0件）12,326件を `scripts/cleanse_phantom_races.py`（冪等・dry-run対応・厳格条件: grade空 AND distance=0 AND date<今日 AND NOT EXISTS結果 AND NOT EXISTS予想）で **物理削除せず** `status='error'` に隔離。条項1検証: error行の予想紐付き=0/結果あり=0/distance>0=0、当日36レース全valid維持。`v_race_mart` は INNER JOIN race_results のため結果0件行は元々ML学習に未到達。影響: src/database/schema.py, src/database/init_db.py, scripts/cleanse_phantom_races.py |
| 2026-06-13 | 【条件戦 grade の前進補完（W-088・v1.14.5-dev）】`jravan_client._grade_from_ra(raw)` 新設＝競走条件名称(JyokenName)域 `slice(27,512)` を cp932 範囲デコードし `_extract_grade` キーワード照合でクラス導出。確定オフセット不要・窓ズレ時は空文字で grade を誤充填しない安全側設計（生RAバイトは未保存・既存RAオフセットは公式JV-Data仕様と不一致のため確定オフセット不可）。`_parse_ra` grade を `_extract_grade(race_name) or _grade_from_ra(raw)` のフォールバックに変更→今後の同期で条件戦gradeが自動充填。既存6,383件（結果あり・grade空・status=valid）の遡及充填は平日バッチ `scripts/fetch_missing_grades.py`（netkeiba二次ソース）で実施予定。影響: src/scraper/jravan_client.py, scripts/fetch_missing_grades.py |
