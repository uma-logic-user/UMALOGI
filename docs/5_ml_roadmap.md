...
| 日付 | 変更内容 |
|------|---------|
| 2026-06-15 | 【内枠複勝バイアスz＋x_trouble_inner三項クロス研究（W-098・v1.16.4-dev）】内枠(枠1-3)複勝率の z スコアを**リークフリー**算出する `src/features/inner_bias.py`（today=当日既走のみ／yesterday=前日全体／基準は全cutoffより前の固定参照）。三項クロス `prev_trouble_proxy × inner_draw × inner_bias` を `research_assemblers.py` で付与し `validate_feature.py --builder inner_bias --each` でゲート検証。**結果: 両版FAIL**（today 勝率33%/-5.9pp、yest 勝率50%/+3.3pp/AUC中立で勝率2/3未達）。前日版が当日版を上回り最有望だが本番不採用（条項8）。**FEATURE_COLS・本番モデル不変**。影響: src/features/inner_bias.py, src/features/research_assemblers.py, scripts/validate_feature.py |
| 2026-06-15 | 【新特徴量WF検証ゲートの標準化＋前走不利クロス特徴量研究（W-097・v1.16.3-dev）】新特徴量の本番採否を一元判定する**ゲートキーパー** `src/ml/feature_gate.py`（`walk_forward_gate`/`summarize_gate`）を新設。既定PASS=ΔROI改善 cutoff>=2/3 ∧ 平均ΔROI>=+2.0pp ∧ 平均ΔAUC悪化<=0.002。CLI `scripts/validate_feature.py`・台帳 `data/feature_gate_results.json`。**CLAUDE.md 条項8**＝新特徴量はゲートPASSなしに本番統合禁止（W-096のノイズ誤認防止を制度化）。前走不利クロス特徴量（×内枠/×同コース複勝/×人気妙味）を `docs/research/cross_feature_ideas.md`＋`scripts/research_cross_features.py` で設計・WF検証中。**FEATURE_COLS・本番モデルは不変**（研究/検証基盤の追加のみ）。影響: src/ml/feature_gate.py, scripts/validate_feature.py, scripts/research_cross_features.py |
| 2026-06-13 | 【U score W-011 クラス変化の grade 供給を前進（W-088・v1.14.5-dev）】U score の相手関係・クラス変化因子（`u_score._calc_competition`／W-010/011）が参照する `races.grade` を、条件戦でも充填できるよう取得側を強化。`jravan_client._grade_from_ra(raw)` が競走条件名称(JyokenName)域を範囲スキャンしてクラスを導出（`_parse_ra` フォールバック配線）→今後の同期で未勝利/新馬/N勝クラス等の grade が自動充填され、`class_change_delta` が前走クラス比較で非ゼロ化する（従来は grade 空で両者ランク5相当→delta=0 縮退）。既存レースの遡及充填は平日バッチ `fetch_missing_grades.py`。**FEATURE_COLS（69列）・モデル係数は不変**（grade はDB側データ供給の改善でモデル挙動変更なし）。⚠️ W-090: `class_change_delta = prev_rank - cur_rank` と docstring「降格馬を高評価」の符号矛盾疑義を発見、週末凍結のため平日に実データ検証して符号確定予定。回帰: tests/test_w088_backfill.py（W-011 delta 非ゼロ化を固定）。影響: src/scraper/jravan_client.py |
| 2026-06-11 | 【過去モデル資産の棚卸しと昇華（v1.12.0-dev）】全保存pkl資産を OOS 400R で静的解析（`scripts/analyze_legacy_models.py`・AUC/スピアマン相関/荒れレースAUC）。結論: ①**卍(EV回帰)が唯一のアンサンブル価値保有**（荒れAUC0.754・ρ(honmei)=0.33・ρ(market)=0.21の真の多様性）→ `src/ml/legacy_ensemble.py` で honmei 勝率に w=0.4 融合し**三連複限定**で本番採用（OOS合計ROI 110.0→119.2%）。②ALPHA(AUC0.81)/ALPHA-Payout(AUC0.82)は ρ(market)≈0.96-0.98 で**市場確率の複製**＝blend_with_market 後の追加情報なし→不採用。③cascade は stage1 pkl 未保存で入力(own_rank1_prob等)再現不能→不採用。④sandbox(AUC0.54-0.64)/pre69feat世代(0.63-0.67)/v2系(0.59-0.70)は精度不足→不採用。キャッシュの market_prob/popularity 全NaN を発見し win_odds から再構成する評価手順を確立。重み決定はtrainフレーム探索→OOS一発検証の2段プロトコル（OOSでの重み選択=リーク禁止を遵守）。影響: src/ml/legacy_ensemble.py(新規), scripts/analyze_legacy_models.py(新規), scripts/backtest_all_tickets.py(拡張) |
| 2026-06-11 | 【完全体アップグレード: 全券種EVエンジン＋AccuracyModelV2移植＋見送り判定モデル（v1.9.0-dev）】①**全券種EV最適化** `src/ml/all_ticket_optimizer.py` 新規: 勝率推計→割引Harville(Benter流・λ2=0.81/λ3=0.65 Lo&Bacon-Shone準拠)で1-3着同時分布→馬連/馬単/ワイド/三連複/三連単の確率→実オッズ(または単勝Shin確率からの控除率込み推定オッズ)比EV≥1.30の歪み抽出→軸-相手フォーメーション生成。Gumbel-MaxによるPlackett-Luce MC検証器同梱(解析式と±0.012一致をテスト担保)。実弾は単複ロック不変＝分析/サブスク用。②**AccuracyModelV2 master移植**: worktree隔離だった的中率特化Classifier(Fbeta=0.5最適閾値・検証データでの閾値決定・リークフリー)を `src/ml/accuracy_model_v2.py` へ移植、orphanテスト6件解消。③**ハイブリッドアンサンブル(EV×Accuracy)検証をmaster移植・実行**: `scripts/evaluate_hybrid_ensemble.py`(honmei 69列整列バグ修正込み)。④**見送り判定** `src/ml/no_bet_filter.py` 新規: レース単位chaos_score(オッズエントロピー/弱い本命/オーバーラウンド異常/モデル-市場JS乖離/構造リスク)≥0.42で見送り。確率・EVは一切改変しない二値ゲート(W-071遵守)。詳細: docs/fable_ultimate_upgrade.md。テスト39件新規。影響: src/ml/all_ticket_optimizer.py(新規), src/ml/no_bet_filter.py(新規), src/ml/accuracy_model_v2.py(移植), scripts/evaluate_hybrid_ensemble.py(移植+修正) |
| 2026-05-31 | 【再学習ルール変更: レース後増分学習を廃止し月曜全件再学習に集約（W-052）】`post_race_pipeline` のレース後増分学習（`IncrementalTrainer.incremental_update`）を本番のレース後評価経路から無効化（`batch_evaluate_date` 既定 `retrain=False`）。理由: `incremental_update`→`_build_partial_df`→`_build_train_df` が**毎レース全年度の特徴量を再生成**し、当日頭数分（24R）繰り返されて約13時間スケジューラをブロックしたため。モデル更新は従来どおり月曜 `weekly_retrain`（全件再学習・土日ガード付き）に一本化する。当日のオッズ/馬体重等のデータ取り込み・的中評価・通知には影響なし。影響: src/ops/retrain_trigger.py, scripts/scheduler.py |
| 2026-05-21 | 【LightGBM 全モデル再訓練完了（69特徴量・Week1-4商用パイプライン適用）】scripts/dry_run_retrain.py 実行（所要時間6,487秒/約108分）。6,138レース/84,930サンプルで honmei/manji/honmei_v2/manji_v2 の4モデルを FEATURE_COLS 69列（x_consensus_score・EV特化特徴量・U score 27列含む）で完全再訓練。HonmeiModel CV AUC=**0.7677**（前回0.7679比 −0.0002・実質同等）。全モデル n_features_in_=69 確認済み。チャンピオンモデル比較は旧68列との不一致でスキップ（正常動作）。特徴量重要度Top3: uf_rank_trend(gain=678)/uf_jockey_win_rate(gain=534)/u_score(gain=498)。影響: data/models/honmei_model.pkl(v20260521_081946), data/models/manji_model.pkl(v20260521_083857), data/models/honmei_model_v2.pkl(v20260521_085834), data/models/manji_model_v2.pkl(v20260521_091844) |
