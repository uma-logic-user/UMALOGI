...
| 日付 | 変更内容 |
|------|---------|
| 2026-06-14 | 【週末サイレント停止の根治＋ハング自己修復（W-093・v1.16.0-dev・社長特例で条項2を上書き）】**真因**: `today_auto_runner.py` のオッズレコーダー/x_scraper が捕捉 subprocess を `errors` 未指定で実行→子(JVLink 32bit由来)の CP932 バイト(0x83)で `_readerthread` が `UnicodeDecodeError` 死亡→パイプ詰まり→300秒タイムアウト空転。「終了しないハング」のため `UMALOGI_SCHEDULER.bat` の終了時再起動が発火せず無限停止。**修正**: ①該当2箇所に `errors="replace"`。②`src/ops/safe_subprocess.py` 新設（捕捉 subprocess の errors=replace 強制）。③`src/ops/heartbeat.py` 新設＝待機/監視/サイクル各ループで鼓動を刻む。④`watchdog.py` に `check_autopilot_heartbeat()` 追加＝auto_runner.pid 生存かつ鼓動 stale(>600s) を「ハング」と判定し強制終了→supervisor が30秒で再起動（自己修復・クールダウン300s・Discord通知）。⑤watchdog の reconfigure を import 安全化。**反映**: 稼働中オートパイロットの再起動で有効。検証: tests/test_self_heal_w093.py 10 PASS。影響: scripts/today_auto_runner.py, scripts/watchdog.py, src/ops/heartbeat.py(新), src/ops/safe_subprocess.py(新)。関連: [[W-085]] |
| 2026-06-13 | 【オートパイロットのCB Soft Stop化（W-087・v1.14.3-dev）】`today_auto_runner._prerace_worker` がCB発動時に直前予想を全スキップしていた挙動を、Soft Stop（予想生成・DB保存・監視を継続、アラートは日次1回dedup）へ変更。env `CIRCUIT_BREAKER_SOFT_STOP=0` でHard Stopに戻せる。影響: scripts/today_auto_runner.py |
| 2026-06-13 | 【金曜夜間バッチJVLink同期の構造バグ修正（W-086・v1.14.2-dev）】`_run_jvlink_sync` のワーカー呼び出しに必須引数 `--fromtime`（当日日付）を追加し、rc≠0時のDiscordアラートを新設（従来はwarningログのみのサイレント失敗）。`record_odds_timeseries.py` にsys.pathボイラープレートを追加しW-080オッズ時系列レコーダーが初めて実稼働。影響: scripts/today_auto_runner.py, scripts/record_odds_timeseries.py |
