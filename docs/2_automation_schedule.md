...
| 日付 | 変更内容 |
|------|---------|
| 2026-05-21 | V1 vs V2 A/B テスト週次レポート自動化完了: `scripts/generate_ab_report.py` を完全実装（対象レース数・ベット数・的中率・ROI・純利益・EV乖離MAE の V1/V2 比較 Markdown 生成、券種別詳細テーブル付き）。`scripts/scheduler.py` の `_JOB_SCHEDULES` / `_CATCHUP_HOURS` に `job_ab_report` を追加（日曜18:00・取りこぼし4時間）。`register_schedules()` にスケジュール登録済み。`_JOB_MAP_FULL` / `_JOB_MAP`（--run-now ab_report）でリカバリー・CLI対応。`tests/scripts/test_ab_report.py` 3件 / `tests/test_scheduler_state.py` 4件追加（計7件 all PASS）。影響: `scripts/generate_ab_report.py`, `scripts/scheduler.py`, `tests/scripts/test_ab_report.py`, `tests/test_scheduler_state.py` |
