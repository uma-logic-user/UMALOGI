...
| 仕様Ver | 日付 | 変更内容 |
|---------|------|----------|
| 1.14.4-dev | 2026-06-13 | **races.grade 列追加で U score 全スキップを根治（W-088・社長特例で条項2解除）**。データ層に additive スキーマ変更: migration #21 `_migrate_add_grade` で `races.grade TEXT NOT NULL DEFAULT ''` を追加。ML層 `u_score._calc_competition` の `SELECT race_id, grade FROM races` が列欠落で例外→`features._add_u_score` が握り潰し U score 18因子+u_score が全予想でスキップしていた障害を解消。スクレイパー層 `jravan_client._extract_grade(race_name)` が確定済み競走名から `G1/G2/G3/OP/L/3勝/2勝/1勝/未勝利/新馬` を決定的導出し `_save_ra` で保存（`scripts/backfill_grade.py` で既存18,864レース→119件充填）。列追加はDBレベルのため稼働中auto_runnerも次予想から即復活。データフロー・実弾ポリシー・モデルpklは不変 |
| 1.14.3-dev | 2026-06-13 | **サーキットブレーカーの Soft Stop 化（W-087）**。W-043日次損失CB（today_auto_runner）とPure_EV CB（prediction.py）を、発動時に予想生成をスキップする Hard Stop から、**予想生成・DB保存・監視を継続しアラートのみ行う Soft Stop** へ変更（env `CIRCUIT_BREAKER_SOFT_STOP` 既定`1`=soft・`0`で旧挙動）。アラートは日次1回dedup。実弾の単複限定ロック・EV閾値（三連系EV≥1.30/Pure_EV EV≥1.15）は安全/エッジ品質ゲートとして不変。本日CB発動で直前予想が9Rに激減していた問題を解消（→29R・Pure_EV 0→8件・premium 300→321シグナル）。データフロー・モデル・実弾ポリシーは不変 |
| 1.14.2-dev | 2026-06-13 | **金曜夜間バッチJVLink同期の構造バグ3件根治（W-086・緊急）**。①autopilot の `_run_jvlink_sync` に必須引数 `--fromtime` を付与（欠落で argparse rc=2 即死＝同期が一度も成功し得なかった）＋失敗時Discordアラート。②SE保存upsertのconflict targetへ `WHERE horse_number IS NOT NULL` を追加（部分UNIQUEインデックス不一致で新規出走馬保存が全滅していた）。③W-080オッズ時系列レコーダーのsys.path欠落修正（初実稼働）。データフロー・モデル・実弾ポリシーは不変 |
